...
- MCP JSON-RPC 的 parse/invalid request/unknown method/invalid params、notification、initialize、tools/list、tools/call、loopback Authorization 都以黑箱驗；`tools/list` 逐元素含順序等於「凍結 catalog 依呼叫者身分收窄後」的那一份，收窄依據是路由表的 `requires`，不是另一份手維名單。完整清單只對能呼叫全部工具的身分成立，不要把 owner 權杖的斷言當成全體的清單。`isError` 對應 HTTP status≥400，structuredContent 只在 JSON object 時存在。
