...
- agent 不直接 `rm` working tree 內容；harness 的 dangerous-rm gate 會讓 headless agent 卡死。**這條規則自 T-162 起多了一道機械保證**：`cli/ocagent/guardbash.go`（`ocagent guard-bash`，由 `buildStatuslineSettings` 掛成 `hooks.PreToolUse`）在指令送到 harness 之前就拒絕「移除目標不是字面路徑」的形狀，成員因此連那個確認提示都碰不到。**它只涵蓋 harness 六種拒絕理由裡的三種**（`possibly-empty variable path`、同款的 command-substitution 變體、`statically-unresolvable target`）；第六種（`— too many command substitutions to analyze`，同一支 `M$` helper、同一個 `circuitBreaker:"dangerousRemoval"`）只在指令替換超過 64 個時觸發，而那種形狀必然含 `$` 或反引號，同一條規則已經擋下；另外兩種（`critical path`、`working directory or its ancestor`）要拿當下工作目錄才判得出來，**不在涵蓋範圍，成員寫出那兩種形狀仍會卡死**——所以下面這套 trash 隔離照舊是必要的。agent 把待刪內容移到 `<workdir>/trash/`，warden 在 spawn／teardown 以 best-effort purge；purge 失敗不可改變 spawn/stop 結果，但任一 containment guard 不過就拒絕並留下 `REFUSED` log。只准清精確的 workdir trash：root/workdir 必須是絕對 clean path、workdir 是 root 的直接子目錄、trash 不能是 symlink，且要比較 realpath 以防 workdir symlink 指到鄰居或 root 外。
