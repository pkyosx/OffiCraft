...
- **開機「推一把」那個迴圈不判定成功，而且刻意不判。** warden 把開機指令貼進成員視窗後會定時按 Enter（`tmuxDeliverNudge`），但它**不讀成員自己的畫面**去確認送出了沒有 —— 那個判定曾經存在、讀的是 claude 狀態列上的 context 百分比，上游改格式之後**恆為 false**，迴圈於是每次都跑滿而沒有任何一層說話。**唯一的成功權威是 server 有沒有在 `StartTimeout` 內收到那顆成員的 `report_waking`**；沒收到時 reconcile 蓋一筆 `wake_timeout` 收據，出現在成員面板的「最近操作」。⚠️ 這**沒有**提升偵測能力：本機按完到 `StartTimeout` 之間仍然沒有任何一層在報告。⚠️ 重試次數**從來沒有被量過**（樹上只有「bounded well under start_timeout」這句話）；它被原封留著是刻意的——要改是另一張票、配它自己的量測。
