...
- agent 不直接 `rm` working tree 內容；harness 的 dangerous-rm gate 會讓 headless agent 卡死。**這條規則自 T-162 起多了一道守衛 —— 它擋掉一部分，不是全部，不要當成完整保證**：`cli/ocagent/guardbash.go`（`ocagent guard-bash`，由 `buildStatuslineSettings` 掛成 `hooks.PreToolUse`）在指令送到 harness 之前就拒絕一部分移除形狀。**它的涵蓋邊界是「拼法」的邊界，不是 harness 那六種拒絕理由的子集**：它只看「移除指令自己那一段有沒有 `$` 或反引號」，所以同一個理由會落在兩邊（`rm -rf $HOME` 擋、`rm -rf ~` 不擋）。**逐條的 COVERED／NOT COVERED 清單寫在 `guardbash.go` 檔頭，並由 `guardbash_test.go` 的兩支邊界測試機械釘住（清單與行為分岔就紅）——不要在這裡複述那份清單，複述正是它前兩次寫錯的原因。** 明確擋不到的有：`find -delete`／`xargs rm`／`/bin/rm` 那族拼法、`if`／`while`／`time`／`nohup` 等前置詞、完全不含展開的目標（`~/build/*`、`cd x && rm -rf ./*`）、以及純字面的危險路徑（`/`、`~`、`..`），**這些形狀成員仍會卡死**——所以下面這套 trash 隔離照舊是必要的。agent 把待刪內容移到 `<workdir>/trash/`，warden 在 spawn／teardown 以 best-effort purge；purge 失敗不可改變 spawn/stop 結果，但任一 containment guard 不過就拒絕並留下 `REFUSED` log。只准清精確的 workdir trash：root/workdir 必須是絕對 clean path、workdir 是 root 的直接子目錄、trash 不能是 symlink，且要比較 realpath 以防 workdir symlink 指到鄰居或 root 外。
