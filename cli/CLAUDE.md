...
- agent 不直接 `rm` working tree 內容；harness 的 dangerous-rm gate 會讓 headless agent 卡死。**T-162 起多了一道守衛，它擋掉一部分、不是全部**：`ocagent guard-bash`（`cli/ocagent/guardbash.go`，由 `buildStatuslineSettings` 掛成 `hooks.PreToolUse`）在指令送到 harness 之前先拒絕一部分移除形狀。**它的邊界是「拼法」的邊界，唯一的權威是 `guardbash_test.go` 的兩張表**——不要在這裡、也不要在檔頭複述那份清單，複述正是它前四輪都寫錯的原因。**擋不到的形狀會走到第二道**：`ocagent guard-permission`（`cli/ocagent/guardpermission.go`，同樣由 `buildStatuslineSettings` 掛成 `hooks.PermissionRequest`）**無條件拒絕**每一個進到它手上的確認提問——搭 `--dangerously-skip-permissions` 時只有跳不過的提示才會進來，進來就代表沒人回答會停住，所以它不解析指令、也不重現 harness 自己的分析；它與 guard-bash 的 fail 方向相反（讀不到 stdin／JSON 壞掉一律仍拒絕），理由是它不在每一支指令的關鍵路徑上。**兩道都是「拒絕」，沒有一道會替成員把東西刪掉**，所以下面這套 trash 隔離照舊是必要的。agent 把待刪內容移到 `<workdir>/trash/`，warden 在 spawn／teardown 以 best-effort purge；purge 失敗不可改變 spawn/stop 結果，但任一 containment guard 不過就拒絕並留下 `REFUSED` log。只准清精確的 workdir trash：root/workdir 必須是絕對 clean path、workdir 是 root 的直接子目錄、trash 不能是 symlink，且要比較 realpath 以防 workdir symlink 指到鄰居或 root 外。
