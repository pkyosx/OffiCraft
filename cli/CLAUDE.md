...
- **換發的門檻不在碼裡，在站台的設定裡。** `auth.warden_credential_lifetime_secs`（壽命，預設 90 天；T-fc53 第二段之前是 30 天）由 warden 每輪 poll 去 `GET /api/machines/credential-policy` 讀，門檻是它的三分之二，再加上一段**由機器 id 決定、每次都一樣**的錯開。讀不到不是錯誤狀態：保留上一次的答案、或內建預設，**照樣換發**——不要把它改成拿不到就停、或每輪印一行。剩下的三分之一是**重試視窗**（15 分鐘一輪），設定的下限就是照它推出來的，不是取整數。⚠️ **這句話在 T-fc53 第二段被推翻，舊版逐字留在這裡**：原本是「這一段的機制只驅動換發，不會讓任何憑證失效：warden 憑證今天仍然沒有 `exp`」。現在 `mintWardenToken` 會蓋 `exp = iat + 壽命`，所以同一個數字既是換發門檻也是死線。**沒換到就是真的掉線**，只能手動重裝；而且站台沒有任何一支 pass 會告訴你哪一台沒換成功。另外：`exp` 在鑄造當下就固定了，所以把設定調小**不會**讓已經發出去的憑證提早死，只會影響之後鑄的。
