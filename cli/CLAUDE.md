...
- **`OC_BASE` 沒設定時 `loadConfig` 會回落到內建 loopback 位址，所以 `cfg.Base` 永遠非空 —— 不要用「`Base` 是不是空字串」當守衛，那個條件恆假。** 要問「有沒有被設定過」看 `Config.BaseConfigured`（`ocagent` 側），它記的是「回落有沒有被走到」，不是「這個值能不能用」。「有設定、但不是 `http(s)://主機` 的形狀」另記在 `Config.BaseMalformed`：`normalizeBase` 是三個模組逐字鏡像的規則，對它無法改寫的值刻意原樣交回，所以形狀檢查不放在那裡。`BaseConfigured` 的零值是拒絕那一側；`BaseMalformed` 的零值是放行那一側（要先設 `BaseConfigured` 才過得了守衛）。兩者都只由 `loadConfig` 產生。
- 每一支讀 `cfg.Base` 的子命令都要對 `OC_BASE` 表態，而**名冊不抄在這裡**：權威是各子命令自己的 `OC_BASE CLASSIFICATION` 註解，`grep -rn "OC_BASE CLASSIFICATION" cli/ocagent` 就是完整清單。新增一支讀 `cfg.Base` 的子命令而沒有那行標題，等於沒有表態。規則有四條，**未設定與形狀不合格一體適用**：拒絕一律指名變數並**不得回印任何值**；拒絕的離開碼跟同處的 `OC_TOKEN` 守衛一致；**長駐子命令（今天只有 `listen`）一律不得拒絕** —— 那裡的「拒絕」就是「離開」，而一顆不再握著連線的成員從外面看跟死掉一模一樣，它改用在既有輸出行上加一段說明（見 `baseAddressOrigin`；owner 2026-09-05 於 `rc-55a969718c98` 裁定）；`context-report` **只能提示、不得拒絕**，它的 stdout 與 exit 0 是 §4 那個 fail-safe 本身，而它每輪對話都跑，改成拒絕就會在每輪炸掉狀態列。
- **加守衛之前先問誰在靠現有的失敗自癒。** 一道拒絕在長駐子命令上等於「離開」，而離開換到的是**沒有訊號**；`listen` 缺 `OC_BASE` 會落進 §5 那套帶訊號的 debounce 重試，換成立即拒絕反而更糟。**這一格已經有裁定了**（owner 2026-09-05 於 `rc-55a969718c98` 圈 [1]，T-89 執行）：答案**兩個候選都不是** —— `listen` 改成**出聲但不拒絕**，在既有的連線行與斷線通知上各加一段說明那個位址是回落來的，**零新狀態、零新印行時機、沒有任何控制流分支在它上面**。⇒ 把它「補成」另外三支那種拒絕，等於推翻一條裁定。
...
- **同一條 poll loop 有兩種醒法，而第三個生產者不是 `Kick`。** `renew` command 走 `updater.RenewNow`：先舉起換發需求、再 `Kick`。接成 `Kick` 會編譯、會跑、什麼都不換 —— 被叫醒的那一輪只會重問一次換發判斷式，而那個判斷式對這台機器的憑證本來就已經回過「還不用換」。這三條接線在 `wireUpdaterSeams`，因為原本它們在 realMain 的長駐分支裡、沒有任何測試進得去。⚠️ **這句的理由換過一次**：原本寫的是「warden 憑證沒有 `exp`，問快到期了嗎一定得到否定」，那在 T-fc53 第一段之前是對的、之後不再是——判斷式改看「發下來多久」（`iat`），沒有 `exp` 也答得出來。結論沒變（`Kick` 仍然換不了憑證），變的是為什麼，而拿去設計解法的是後者。
- **換發的門檻不在碼裡，在站台的設定裡。** `auth.warden_credential_lifetime_secs`（壽命，預設 30 天）由 warden 每輪 poll 去 `GET /api/machines/credential-policy` 讀，門檻是它的三分之二，再加上一段**由機器 id 決定、每次都一樣**的錯開。讀不到不是錯誤狀態：保留上一次的答案、或內建預設，**照樣換發**——不要把它改成拿不到就停、或每輪印一行。剩下的三分之一是**重試視窗**（15 分鐘一輪），設定的下限就是照它推出來的，不是取整數。⚠️ **這句話在 T-fc53 第二段被推翻，舊版逐字留在這裡**：原本是「這一段的機制只驅動換發，不會讓任何憑證失效：warden 憑證今天仍然沒有 `exp`」。現在 `mintWardenToken` 會蓋 `exp = iat + 壽命`，所以同一個數字既是換發門檻也是死線。**沒換到就是真的掉線**，只能手動重裝；而且站台沒有任何一支 pass 會告訴你哪一台沒換成功。另外：`exp` 在鑄造當下就固定了，所以把設定調小**不會**讓已經發出去的憑證提早死，只會影響之後鑄的。
...
- **開機「推一把」那個迴圈不判定成功，而且刻意不判。** warden 把開機指令貼進成員視窗後會定時按 Enter（`tmuxDeliverNudge`），但它**不讀成員自己的畫面**去確認送出了沒有 —— 那個判定曾經存在、讀的是成員狀態列上的 `"% context"` 字樣，而它**恆為 false**，迴圈於是每次都跑滿而沒有任何一層說話。🔴 **那條狀態列不是「別人家的」，是我們自己畫的** —— `buildStatuslineSettings`（`cli/ocwarden/spawn.go`）把成員的 statusLine 指到我們自己的 `ocagent context-report`，而 T-51a8 把它的版面從 `🧠 N% context` 換成 `<bar> N%`（沒有 `context` 這個字）。⇒ **教訓不是「不要讀別人的畫面」，是「我們自己兩個模組隔著一個沒有契約的字串耦合，而沒有任何一層在守它」。** 讓 `ocagent` 主動發佈一個給 warden 讀的標記是合法的設計，只是本票選了更好的權威（server 的收據）。**唯一的成功權威是 server 有沒有在 `StartTimeout` 內收到那顆成員的 `report_waking`**；沒收到時 reconcile 蓋一筆 `wake_timeout` 收據，出現在成員面板的「最近操作」。⚠️ 這**沒有**提升偵測能力：本機按完到 `StartTimeout` 之間仍然沒有任何一層在報告。⚠️ 重試次數**從來沒有被量過**（樹上只有「bounded well under start_timeout」這句話）；它被原封留著是刻意的——要改是另一張票、配它自己的量測。🔴 **而且改它會踩到另一個模組**：那個迴圈每次 spawn 固定花掉 `nudgeMaxAttempts × nudgeSettle`＝30 秒，這 30 秒被 `server/ocserverd/receipt_watch.go` 的 `receiptDeadlineSecs`（90 秒）花掉，算完只剩約 5 秒餘裕。兩個常數跨 module（`cli/ocwarden` 有自己的 `go.mod`），**沒有任何機械保證**——`cli/ocwarden/spawn_test.go` 用字面值釘住 30 與 1s 並指回那裡，那個釘子是唯一的連結。
- agent 自己跑 `rm` 可能撞上 harness 的 dangerous-rm 確認提示，沒人按就會讓 headless agent 卡死。**T-162 起多了一道守衛，它擋掉一部分、不是全部**：`ocagent guard-bash`（`cli/ocagent/guardbash.go`，由 `buildStatuslineSettings` 掛成 `hooks.PreToolUse`）在指令送到 harness 之前先拒絕一部分移除形狀。**它的邊界是「拼法」的邊界，唯一的權威是 `guardbash_test.go` 的兩張表**——不要在這裡、也不要在檔頭複述那份清單，複述正是它前四輪都寫錯的原因。**擋不到的形狀會走到第二道**：`ocagent guard-permission`（`cli/ocagent/guardpermission.go`，同樣由 `buildStatuslineSettings` 掛成 `hooks.PermissionRequest`）**無條件拒絕**每一個進到它手上的確認提問——搭 `--dangerously-skip-permissions` 時只有跳不過的提示才會進來，進來就代表沒人回答會停住，所以它不解析指令、也不重現 harness 自己的分析；它與 guard-bash 的 fail 方向相反（讀不到 stdin／JSON 壞掉一律仍拒絕），理由是它不在每一支指令的關鍵路徑上。**兩道都是「拒絕」，沒有一道會替成員把東西刪掉**；成員要刪東西時，開機文件教的唯一做法是 `rm -rf <完整路徑>`（寫法細節以 `seeds/` 為準，這裡不複述）。`<workdir>/trash/` 是已退役的「移進 trash、由 warden 刪」做法留下的目錄，warden 仍在 spawn／teardown 以 best-effort purge；purge 失敗不可改變 spawn/stop 結果，但任一 containment guard 不過就拒絕並留下 `REFUSED` log。只准清精確的 workdir trash：root/workdir 必須是絕對 clean path、workdir 是 root 的直接子目錄、trash 不能是 symlink，且要比較 realpath 以防 workdir symlink 指到鄰居或 root 外。
...
- **`ocwarden` 對 `OC_BASE` 不猜：沒設定就拒絕，`install` 與 `run` 兩端各自拒絕。** ⚠️ **這一條只講 `ocwarden`** —— `ocagent` 的 `loadConfig` 今天**仍然**在 `OC_BASE` 為空時回填預設位址，不要把這一條讀成整個 `cli/` 的契約。`install` 拒絕安裝並非零退出（那個猜測會被寫進 plist，之後每次啟動都繼承）；`run` 停下來但**不退出** —— 不 spawn、不連線、不發任何請求，落一個 sentinel 說明原因。判斷式看的是 **env 有沒有被設定**，不是最終值長什麼樣：站台主機合法地把 `OC_BASE` 設成 loopback，拿值去比預設常數會拒絕那台機器。⚠️ **halt 過的 warden 不會自己好**（事後設對也要重啟）——這是刻意的，站台網址不是會自己到位的東西，「用到時才決定」那條修法在這裡不適用。⚠️ **那台機器仍然會出現在名冊上**（名冊列在 install 之前就建好了），只是永遠不上線；server 端的 onboarding 已經有一句寫給讀者看的（`wake_undispatched`）。設計理由、以及「退出之後 launchd 到底會不會重啟」那個**尚未裁決、而樹上有三份說法**的問題，寫在 `cli/ocwarden/basegate.go` 檔頭，改這一格之前先讀它。
