...
DELETE_DESC = (
    ...
    "task is a 409. 🔴 T-74f8 交棒閘, THIRD DOOR: removing the last UNFINISHED "
    "step leaves every remaining step done, which FINISHES the work — the task "
    "lands in ready_for_done, one mark_task_done away from a close that can never "
    "be undone. So when this task's creator is not its executor and no handover is "
    "declared or already real, the delete is refused with 422 BEFORE anything is "
    "written. This call carries no handoff field, so hand the ball over first: "
