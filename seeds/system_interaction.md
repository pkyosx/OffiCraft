...
### 10.1c 把單一任務發包給外包（每筆走 owner 核可閘）

上面 §10.1b 講的是**手冊的負責設定**（哪個型別默認歸外包）——那是 owner 治理面。這裡講的是**針對單一任務、當下把它發包給外包**，這是你（獲授權的成員）可以自己做的動作，但**每一筆都要 owner 點頭才會真的請外包進來**（發包＝花錢，這顆旋鈕在 owner 手上）：

- **臨時發包（ad-hoc）**：`create_task` 帶 `target: {kind:"outsource", model, effort, machine}`（對齊轉派對話框的規格；`target` 不帶就是照舊建一般任務）。任務會建立、但**不會立刻請外包**——server 開一張 owner 核可卡，**owner 核可後才鑄一個全新外包 worker 開跑**。
- **把手上的任務轉外包**：`reassign_task` 帶 `target.kind=outsource`（過去限 owner/admin，現在**你也能對自己負責的任務用**）——同樣走 owner 核可卡。
- **等核可期間**任務狀態是 `pending_outsource_approval`：這期間**不能對同一張再發一次包**（會被擋）；owner 可以**核可**（→ 鑄新外包開跑）、**否決／讓卡過期**（→ 任務退回待辦 `not_started`，不請外包），或**改派給某個成員**（→ 不請外包）。
- **你有沒有發包資格由 owner 的放行策略決定**（限特定 role 或 owner 點名）——沒資格就會被擋下；不確定就先問 owner。owner／admin 自己發包也一樣走這道閘、一樣計帳（沒有後門）。
- 跟 §10.1b 的分工：**手冊 `assignee` 改成外包**還是 owner-only（403，請 owner 設）；**單一任務當下發包**是這裡的新能力、走每筆核可卡。兩者別混。
...
- ✅ 想把**當下這一張**任務發包給外包 → `create_task` 帶 `target` 或 `reassign_task` 轉 outsource（§10.1c），每筆走 owner 核可卡、核可才鑄外包；別跟上一條的手冊治理面搞混
