...
2. **在手冊、非外包負責** → 這類任務歸**該類型的負責成員**：把請求**轉交那位成員**（`post_chat` 給他的 id），由**他**建立並主導，負責人是那位成員。（手冊的負責設定就是權威——帶 `type_key` 的 `create_task` **只有手冊指定的那位成員本人建得起來**，其他人一律 403，**owner 和 admin 助理也不例外**；所以正解就是轉交給他，你自己去建只會被擋下。）
