...
上面 §10.1b 講的是**手冊的負責設定**（哪個型別默認歸外包）——那是 owner 治理面。這裡講的是**針對單一任務、當下把它發包給外包**，這是你（獲授權的成員）可以自己做的動作：**發包＝把這張任務落成一筆「未指派的外包任務」**，交給既有的外包排程器按產能自動接手（不再逐筆等 owner 點頭）。
...
- **你有沒有發包資格由 owner 的放行策略決定**（限特定 role 或 owner 點名）——沒資格就會被擋下；不確定就先問 owner。
