...
| **agent** | 讀自己的身分、照開機程序走、自報狀態 | 決定自己何時被起，也不持有那條長連線。它連自己有沒有被派過都不知道 |
...
worker 那邊**至少四種**：**排程收案時的 admission**、**owner op**（換機器／喚醒**且 session 已經不在**，見§一的條件）、**週期 tick 對「已指派」worker 的重新投遞**，以及**救援 START**（走正職那顆 FSM）。

### 🔴 「收尾中按喚醒」現在靠的是第四種，不是第二種

這是 T-65 包④ 換掉的那條路，而且它換得**看不出來**——兩種寫法對 owner 來說都是「按了它就回來」：
...
**例外是舊 session 佔位（`session_already_exists`）**：那一次被拒的 START 不 bench；之後的強制停止（zombie takeover）會 bench 那台，但**只到那台機器回報停止成功為止**（殺掉或 `no_such_session` 都算）——收到回報的下一輪就在同一台重新開機。停止失敗、回報的不是那台、或同一個 worker 在冷卻期內又被強制停止一次（舊 session 反覆出現），bench 照常等冷卻結束。
...
🔴 **不要把它讀成「按不動就再按」**（T-82 前這份文件就是這樣寫的，而那句話從來沒有成立過）。那個迴圈**分辨不出 Enter 有沒有被接受**，它只是無條件按滿上限——上限是 30 次、每次隔 1 秒，所以**每一次 spawn 都固定花掉 30 秒**，不論第一下有沒有成功。曾經有一個「送出了沒有」的判定，讀的是成員狀態列上的字樣，而它恆為 false（機制見 `cli/CLAUDE.md` §5）。⇒ **這一戳成功與否的唯一權威在 server**：有沒有在 `StartTimeout` 內收到那顆成員的 `report_waking`。
...
⇒ 兩種 runtime 的開機文件仍然不能互換：只有 codex 那份要求成員在開機那一輪結束時把控制權交回 sidecar。
...
**worker 的退場方式不同**：開機失敗會把那台機器 bench 掉、冷卻一段時間，下一輪落到放置的第三層（見第四節；舊 session 佔位那一種的例外也在第四節）。
...
- 清掉前一代的 session 錨（⚠️ 唯一的例外：warden 以「session 已經存在」拒絕這道 START 時，server 收到回執會把它還原——舊 session 還活著，錨仍是它的）
- 蓋上喚醒錨
- 寫進那條下行通道（**這個方向沒有 ack**）
...
- warden 的拒絕理由集合
- **開機程序那兩份 seed**（兩種 runtime 各一份，不能互換）
- 喚醒錨的 writer 與四個清除點
