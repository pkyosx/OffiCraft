...
**例外是舊 session 佔位（`session_already_exists`）**：那一次被拒的 START 不 bench；之後的強制停止（zombie takeover）會 bench 那台，但**只到那台機器回報停止成功為止**（殺掉或 `no_such_session` 都算）——收到回報的下一輪就在同一台重新開機。停止失敗、或回報的不是那台，bench 照常等冷卻結束。
