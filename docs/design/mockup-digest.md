...
> ➡️ **對 LIVE 的意義（刻意分歧，不要照抄）**：那顆 `MB-1IF746` 徽章是原型當年的樣子，**LIVE 已經不這樣做了**。owner 2026-08-15（回覆卡 `rc-2a6b96d0fb0c` ①）裁定身分徽章一律顯示**真 id**（`mira` / `m-…` / `ow-…`），因為推導出來的 `MB-` 標籤查不到任何東西、卻讓同一個人有兩套識別；`member_no` 這個 wire 欄位也已整條退場（`rc-ae0ba9565c99` ①）。這一段保留只為記錄原型長什麼樣。
