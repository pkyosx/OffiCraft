...
- Dismissing a member deletes that member's rows in every theme. Releasing an
  outsource worker does NOT run a prune: the worker row stays in the roster as
  released, and its rows stay with it. They are unreachable from the cockpit
  (a released worker renders no chooser) and they go when their theme goes.
...
- Removal: removing an image clears only its own selections and moves nobody
  else; deleting a theme clears only its own; a dismissed member leaves nothing
  behind.
