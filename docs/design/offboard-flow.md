...
⚠️ **這個「再送一次」今天也涵蓋下線那條（T-ed79 起）。** 決策迴圈那一臂確實不派——下線那一臂在成員還線上的整段期間**什麼都不派**（見第三節）。但它的兩個收口——成員自報收完、owner 按強制下線——送出的**同時就記下一道 at-least-once 標記**（`reconcileState.RobustStopPendingAt`，連 fail-closed 的閘當場擋掉那一幀時也照記），排程迴圈看到成員過了 `stop_retry` 還在線就再送一次。T-253 之後這道 stop 還多了一條有序的目標鏈、鏈尾是廣播（見第五節），所以「沒有任何來源叫得出機器名字」也不再是它掉單的方式。⇒ **frame 掉了不再只剩 owner 的手。**
