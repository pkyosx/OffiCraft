...
sequenceDiagram
    ...
    Note over rec,w: 而這一條的重送今天涵蓋下線：<br/>兩個扳機送出時都記下 at-least-once 標記，<br/>成員過了 stop_retry 還在線就再送一次
```
...
⚠️ **「至少一次」那一半靠的是送出當下記下的標記，不是決策迴圈會不會派命令。** 下線那條自己沒有決策臂，它的至少一次來自那道標記（見上），所以它同樣蓋得住通道的「至多一次」。**真正沒有自動補償的是再上一層：stop 一直送得到、session 卻撐得過去——那一格才是 owner 的手。**
...
| **任務結案收尾（外包限定，T-244）** | 外包被綁的那個任務落到終局——四種結案方式都算（執行者結案、終止、標重複、強制結案） | **有時鐘，而且是這條路徑唯一自己起的時鐘**：`task.close_winddown_secs`（預設 300 秒），從結案那一刻的 `stopping_since` 起算。它跟加速停止**共用範圍、不共用數字** | **不重生**。窗口結束就釋出離隊（列 released）＋收走 session |
...
### 🔴 任務結案也給外包一段收尾（owner 2026-09-20，rc-604d8fc39cfd 圈 [0]）

⚠️ **這一段推翻了 T-182 的裁定**（rc-571b665bc047「外包改在按下結案那一刻遣散」）。那
條的理由是「任務一旦終局，外包就不被允許再對它寫任何東西，留著只是佔額度」——這句話
對**任務**成立，對**它自己的 session** 不成立：推 commit、收回 sub-agent 的結果、寫自
己的傳承，都不是對任務的寫入，也都不是瞬間的事。而且四扇門裡有兩扇（強制結案、終止）
本來就會落在一個**還沒走到 `ready_for_done`** 的外包身上，那正是 T-182 自己在註解裡寫
下的代價。

現在結案**開一個窗**而不是關掉一個：寫 `desired_state=offline` ＋ `stopping_since` ＋
`refocus_op=task_close`，畫面沿用既有的**停止中**（不新增狀態），列還留在名單上。收口
在 tick，兩條臂取先到者：

* **站台自己判定那個工作階段真的不在了**（`workerSessionConfirmedGone`——跟 `停止` 那條
  臂同一個連續離線窗）。外包呼叫 `report_stopped` 是**要求提早收**，
  🔴 **那個回報只是請求，不是證明**：收口仍然等站台自己的離線判定。owner 同一張卡裁定
  這個「確認」就用**站台現有的離線判定**，不要另外開一條「站台去問那台機器」的路。
* **窗口到期**（`task.close_winddown_secs`）。

結案當下就已經離線的外包沒有東西可收，**當場**釋出＋收走，跟 T-182 之後的行為一樣。
...
**先問：這道 stop 送去哪一台？** 殺這條路徑解析的是**那個 session 實際所在**的機器，不是它被指定該在的那台——這兩者在換機器時會不同，而搞混的後果是舊 session 永遠殺不掉。它是一條**有序的鏈**，第一個「叫得出機器名字」的來源就贏，正職與外包共用同一支解析（`server/ocserverd/shutdown.go` 的 `killTargetChain`；**順序以碼為準，不在這裡抄死**）：

- **外包**：這一輪 server 記得自己把開機派到哪 → 它此刻的 live 連線 → 它上一次確認落地的那台（`last_machine_id`）→ 廣播。
- **正職**：它此刻的 live 連線 → 它上一次確認落地的那台（`last_machine_id`）→ 它被指定該在的那台（pin）→ 廣播。

⚠️ **`last_machine_id` 排在正職的 pin 前面，這是刻意的**：pin 講的是「接下來該跑在哪」，換機器之後它已經指向新機器，而要殺的那個 session 還在原機；「上一次確認落地」才是在講那個 session 現在人在哪裡。pin 仍留在鏈上，是因為一個從沒落地過的離線成員只剩它可問。

⚠️ **鏈尾是廣播**：一個名字都叫不出來時，這道 stop 會送給**所有在線的 warden**。那是安全的，理由跟身分掃蕩一樣——命令是用那個 session 自己的名字定址的，沒有那個 session 的機器收到就是一個乾淨的 no-op，砍不到別的東西。真正「什麼都不送」只剩一種情況：**整個機隊沒有任何 warden 在線**。

⚠️ 這是**殺路徑專用**的解析，不是通則：其他非停止類的命令送往指定的那台；外包**開機**的 `desired_machine_id` 更是 hard pin，不可用就原地卡住、不 fallback，兩套規則不要互相套用。（連線時的身分掃蕩則是另一種形狀：它一開始就**只**送給所有**其他** online 的 warden，而且刻意不追回執。）
...
**連 SSE 那道門也是不對稱的**，而這是這節最強的證據：擋回連線的那道 gate 對外包**直接放行**——一個已結案的 worker，它的 session 要活到結案流程走完。worker 的停止意圖靠排程那一側的狀態壓著執行，**不靠這道門**。
