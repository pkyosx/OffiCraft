...
同一支 `report_stopped`，**兩種身分走同一個判斷**（T-251，owner `rc-b08d49dc3b03`）：第一次報停一律收，在同一支呼叫裡就把停止送出去；要不要重生由 desired_state 單獨決定。重複呼叫回 `already_reported`，什麼都不做。**不對稱只剩下那一刀怎麼砍**：正職送 robust STOP，外包送 worker kill，走的是 worker 自己的回收路徑。
