...
## 完整例子：Slack 進來一則訊息 → review PR → 回覆意見
...
> 有人在 Slack 丟一個 PR 連結進來，要有人去看完、給出可執行的意見、把結論回到那串 thread 上。
...
> 🔑 **識別鍵是防重複的機制。** 同一個 `pr_url` + `slack_thread` 再進來一次，系統會認出「這件事已經有一張卡了」，不會開第二張。
> 識別鍵可以複合——像這裡就是兩個欄位一起當鑰匙。
...
5. 回到 Slack thread 貼結論摘要，並在 PR 上留 approve / request-changes。
...
Slack 那則訊息透過**回呼端點（webhook）**打進來，或你直接在座艙丟一句「幫我看這個 PR」。
...
| 7 | 回貼 Slack + 在 PR 上留結論 | thread 上有摘要、PR 上有 approve 或 request-changes |
