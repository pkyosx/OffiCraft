...
| 元件 | 是什麼 | 職責 |
| --- | --- | --- |
| **ocserverd**（server） | 一支 Go 服務 | **唯一真相來源**：身分、聊天、卡片、任務、傳承全在它上面。同時提供三個介面（REST + SSE + MCP），內嵌控制台網頁，資料存本機 SQLite。**只綁 loopback。** |
...
> 兩者以前在圓點上長得一模一樣。等成員旁邊的連線程序接上 SSE 就是 **online**；若在 **`WakingTTLSecs`** 內始終沒連上，這次喚醒算失敗，
...
| **發（成員打 server）** | **MCP** | 成員**主動做事**都走這裡（`POST /api/mcp`，JSON-RPC 2.0 的工具呼叫）：送訊、開卡、建任務、回報節點做完、查名冊、寫傳承。 |
...
角色記憶（角色誌與傳承）、任務進度全在 server 上（見上一節）。於是 session 變成純消耗品：丟掉一手、換上一手，
**該記得的一個字都沒少。**
...
出廠版的形狀大致是：報 stopping（告訴 server 我要退場了，presence 進入收尾狀態）→ 把任務進度與步驟備註寫回 server → 給自己留一份交接 baton → 收拾暫存 → 報 stopped。**細節以那份文件的現況為準**，這裡只是讓你知道有這麼一段。

**幾乎每一種都沒有時鐘。** 會開始倒數的**只有三種**。前兩種都叫「加速停止」：**context 升到第二段門檻**（它真的快撐不住了，寧可
硬收，也不讓一個半死的 session 卡在那裡占著位子），以及**你自己按下加速停止**；這兩種共用同一個秒數（預設 **120 秒**，可在
設定調，見 [設定與參數](settings.md)）。第三種**只發生在外包身上**：**它的任務結案了**——這時它拿到的是另一個秒數
（**任務結案收尾秒數**，預設 **300 秒**，同樣在設定調），因為「外包關機要多久」跟「你自己那條升級路徑要多久」是兩個決定。
三種都一樣：逾時沒收完 server 就**下令強制回收**、由 warden 執行。
...
處在 **waking（喚醒中）**（server 派出開機命令當下就記下了），等它旁邊那個連線程序接上 SSE 長連線（`listen`）就算
**online**——這時它就正式接棒了。
