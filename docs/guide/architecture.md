...
> **一切都在你的 Mac 上。** server 是唯一真相源、只綁 loopback；每台機器上有一個 warden，照 server 的指令
...
![架構圖：owner 透過控制台指揮，所有連線收斂到唯一真相源 server；每台機器上一個 warden 依 server 指令起停跑在 tmux 裡的成員](assets/architecture-overview.svg)
