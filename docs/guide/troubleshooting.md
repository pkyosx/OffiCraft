...
🔴 **如果你看到的是那個框，它列的兩條建議在這一步都幫不上忙，還會把你帶開**：
第一條叫你去「監控」看機器在不在線——**這台機器是好的**（warden 裝得起來，這一節的前提就是它裝好了）。
第二條叫你以「最近操作」那一行為準、說它比框裡精確——但那一行是**你改設定之前**留下的舊紀錄，上面寫的是 `claude_bin_unresolved`（叫你去裝 `claude`），**它不會因為你改了執行環境而更新**。照它做就繞回原點了。

⇒ 這一步先把 `codex` 登入起來再重試；還是不行，才回頭走上面那兩條建議。之後在這台機器上新請的成員也要一併改——新成員預設同樣是 Claude Code（見 [成員與外包](members.md)）。
