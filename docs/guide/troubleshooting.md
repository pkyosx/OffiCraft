...
第一條叫你去「監控」看機器在不在線——**這一條值得瞄一眼，但多半不是你這次的答案**。它不是廢話：機器離線／warden 沒連上**本來就是這個框的頭號成因**（上一段剛講過）。只是這一節的前提是你剛在這台機器上把 `codex` 裝起來，所以它當時是通的；**先看一眼監控，那邊沒問題就往下走**。
第二條叫你以「最近操作」那一行為準、說它比框裡精確——但那一行是**你改設定之前**留下的舊紀錄，上面寫的是 `claude_bin_unresolved`（叫你去裝 `claude`），**它不會因為你改了執行環境而更新**。照它做就繞回原點了。

⇒ 監控那邊沒問題的話，這一步就是先把 `codex` 登入起來再重試。**監控那邊真的有問題**（機器離線、warden 沒連上），那就先修那台機器，這一節的問題要等它通了才看得出來；**登入了還是同一個框**，才回頭走「最近操作」那條、看它這次寫的是什麼。之後在這台機器上新請的成員也要一併改——新成員預設同樣是 Claude Code（見 [成員與外包](members.md)）。
