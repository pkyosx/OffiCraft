...
把它改成 Codex 就對得上了：開 Mira 的詳情面板，按**喚醒／更改**，在那份啟動設定裡把「AI 執行環境」選成 **Codex**，確認送出。⚠️ **`codex` 要先登入**——裝了但沒登入的話，改完仍然起不來，而且症狀跟改之前長得一樣。之後在這台機器上新請的成員也要一併改——新成員預設同樣是 Claude Code（見 [成員與外包](members.md)）。
