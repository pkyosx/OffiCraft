...
把它改成 Codex 就對得上了：開 Mira 的詳情面板，按**喚醒／更改**，在那份啟動設定裡把「AI 執行環境」選成 **Codex**，確認送出。⚠️ **`codex` 要先登入**——裝了但沒登入的話，改完仍然起不來，**但症狀不一樣、而那個不一樣正好是線索**：按下確認的當下會跳出一個標題寫「**這次沒有送出喚醒指令**」的框，而「最近操作」那一行不會有新的內容。看到那個框就是這一種。⚠️ 那個框會建議你去「監控」看機器在不在線——**這一種情況下那條不適用**，機器是好的，要補的是 `codex` 的登入。之後在這台機器上新請的成員也要一併改——新成員預設同樣是 Claude Code（見 [成員與外包](members.md)）。
