...
- **AI 執行環境**〔成員面板 / 外包詳情〕— 這位成員底下跑的是 **Claude Code** 還是 **Codex**。**這一格是 agent 回報過的，從來沒回報過就是 `—`**——你剛把它設成 Codex、還沒開機起來時，這裡不會馬上變。⚠️ **而「從來沒開機成功過」的那位成員，改了之後這裡不但不會變，旁邊也不會出現待生效的提示**（提示要先有一個回報值可以比對才畫得出來）——**那不表示沒存到**，下次啟動就是照新的那個跑。⚠️ 它跟 MODEL、EFFORT **不是**同一套：那兩格在成員停掉之後會清空，這一格會**留著上次回報的值**。要改走喚醒／更改那一區的啟動設定，怎麼挑見 [成員與外包](members.md)。
- **EFFORT · Thinking（思考強度）**〔成員面板〕— 這個 agent 的推理強度（低／中／高／最高），下次喚醒或換手時生效。**這一欄跟 MODEL 同一套規矩：顯示的是 agent 回報過的值，而且成員停掉之後會清空**，你新設的那一個要下次啟動才套用、也才會在這裡看到。（外包詳情裡對應的欄位叫 **Effort（投入度）**。）
