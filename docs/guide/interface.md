...
- **AI 執行環境**〔成員面板 / 外包詳情〕— 這位成員底下跑的是 **Claude Code** 還是 **Codex**。**跟 MODEL 同一套規矩：這一格是 agent 回報過的，沒回報過就是空的**——你剛把它設成 Codex、還沒開機時，這裡不會馬上變（那次設定會另外標成待生效）。要改走喚醒／更改那一區的啟動設定，怎麼挑見 [成員與外包](members.md)。
- **EFFORT · Thinking（思考強度）**〔成員面板〕— 這個 agent 的推理強度（低／中／高／最高），下次喚醒或換手時生效。**這一欄跟 MODEL、AI 執行環境同一套規矩：顯示的是 agent 回報過的值**，你新設的那一個要下次啟動才套用、也才會在這裡看到。（外包詳情裡對應的欄位叫 **Effort（投入度）**。）
