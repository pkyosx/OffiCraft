...
- **EFFORT · Thinking（思考強度）**〔成員面板 / 外包詳情〕— 這個 agent 的推理強度（低／中／高／最高），下次喚醒或換手時生效。**這一欄跟 MODEL 同一套規矩：顯示的是 agent 回報過的值，而且在成員面板，成員一停掉這一格就清空**（**外包詳情**不清空，跟上面那條一樣三格都留著），你新設的那一個要下次啟動才套用、也才會在這裡看到。（外包詳情這一格**同名**，也叫 **EFFORT · 思考強度**；會看到「低投入／中投入」那種字樣的是**任務卡**上外包執行者那條 `代號 · 模型 · 投入度`，不是這一格。）
