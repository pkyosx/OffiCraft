...
> 有人丟一個 PR 連結進來（Slack、你自己在座艙貼、或任何入口都算），要有人去看完、給出可執行的意見。
...
> 如果把 `slack_thread` 也放進識別鍵，同一個 PR 從另一串 thread 被提起、或你直接在座艙貼連結（根本沒有 thread），
...
連結可以從**回呼端點（webhook）**打進來（例如接上 Slack），也可以是你直接在座艙丟一句「幫我看這個 PR」——**對任務來說是同一件事**，入口不影響它的身分。
