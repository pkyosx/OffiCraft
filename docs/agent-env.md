...
PATH=/opt/homebrew/sbin:/Users/you/.local/bin:$PATH   # ⚠️ 見下面「不展開」
```
...
❌ PATH=/opt/homebrew/sbin:$PATH    # 值是字串 "/opt/homebrew/sbin:$PATH"
✅ PATH=/opt/homebrew/sbin          # 只寫你要「加」的,系統 PATH 會自動接在後面
```

> PATH 特別說明:你在這裡寫的 PATH 會**先**被載入,agent 的 workdir 和原本的
> 系統 PATH 會接在它後面,所以你只需要寫「要補上的那幾條」。
...
```bash
grep "agent env" ~/.officraft/log/ocwarden.err.log
```
