...
- 🔴 **新增 migration 的號碼必須大於 `origin/main` 目前宣告的最大號**，兩個來源都算（`migrations/*.sql` 與 `goose.AddNamedMigration*` 註冊的 Go migration）。跳過的號碼永遠留著不補：低於已釋出最大號的新號會讓**已經在跑的站**下次開機在 `runMigrations` 停住、exit 1、不會 listen，而全新安裝完全看不出來（空白 DB 上「缺號」不存在）。**同樣不可逆的另一半：已經釋出的 migration 檔案內容不可以再改**——goose 一個版號只記一次、永不重跑，改動只會落在全新安裝上，兩種站從此 schema 不同而且沒有任何錯誤。要改就開新號。**這一半兩個來源都會紅**：`migrations/*.sql` 比對整個檔案，Go migration 比對它**整份原始碼檔**（不只 up/down 函式主體）——所以一個純機械的改動（改名波及、gofmt）也會叫，那時要由人決定怎麼辦，而不是由這道檢查默默放行。
  ⚠️ **「大於 main 的最大號」是必要條件，不是充分條件**：兩條同時在飛的分支各自對著同一顆 main 取 max+1 會取到同一個號。取號前仍要掃所有遠端分支已占用的號——**但那只是省事，不是防線**：`TestMigrationVersionNumbersAreClaimedByExactlyOneSource` 要等兩邊會合之後才看得見，而那時已經在主線上了。防線是 `server/ocserverd/migration.lock`。
...
```
  ...
  **衝突了怎麼解**：不要用手接文字。先確認兩邊的版號真的沒撞（真的撞了就把後進來的那支 migration **檔案本身**改名到 max+1），再跑 `./bin/gen-migration-lock`——它從工作樹重新列舉並整份重寫，`roll` 是算出來的，**任何情況下都不得手改**。改完看 diff：新 migration 只能是**尾端新增的行**；**中段有行被改動 ＝ 有一支已釋出的 migration 被編輯或刪除了**，那時要停下來查，不是繼續 commit。
  ⚠️ **重新產生 lock 不等於那次改動被允許。** 編輯一支已釋出的 migration 之後跑產生器，`TestMigrationLockMatchesTheTree` 會變綠——因為 lock 現在**誠實地描述了這棵樹**（實測：改 `00022` 一行註解 ⇒ 未重新產生時紅在 `[lock:content]`，重新產生後 `ok`，而 lock 中段第 42 行的雜湊變了）。說「不准」的是拿本樹跟 `origin/main` 比的那半（`TestMigrationLockGrowsOnlyAtItsTail`，要求 main 的行是本樹的**精確前綴**）與 T-64 的升級路徑檢查。🔴 **這兩者在 main 分支上都是空轉的**（本樹就是 main，沒有東西可比）——**它們只在 PR 路徑上活著**，而中間那一段唯一的守衛是**讀 diff 的那個人**。
  上面幾條由 `TestAStationAtTheReleasedVersionCanUpgradeToThisTree` 與 `TestMigrationLockGrowsOnlyAtItsTail` **兩支**釘住：前者照 `origin/main` 重播出一顆站台狀態的 DB 再跑 production 的 `runMigrations`，後者要求 main 的 lock 行是本樹 lock 行的精確前綴。**兩者共用同一個 baseline 解析**（第二種解法就是第二個會漂移的東西），所以性質也一樣：**它們讀的是本機的 `origin/main` ref、不會自己 fetch**，所以本機跑之前先 `git fetch origin main`——ref 過期只會讓它們變寬鬆、不會變嚴格。解析不到 `origin/main` 時本機 skip 並說明（**skip 不是綠**），在 CI 則是直接紅，因為 CI 沒有基線就等於這兩道檢查被關掉。CI 那一側靠 `go-checks` job 的 `actions/checkout` 帶 `fetch-depth: 0` 才有那個 ref，**那一行是承重的**。
