...
- 🔴 **一道 kill 要嘛「瞄準了某台」要嘛「廣播出去」，記帳必須分得出來**（`workerStopDispatch.Target` vs `.Fanout`）。廣播的前提就是沒有任何來源說得出機器，所以它打到的每一台都可能從沒 host 過那個 session：把其中任一台塞進 `Target` 會讓那台一句例行 `no_such_session` 解除整個重送（＝ 殘活 session），塞空字串又會讓重送判定恆 false（＝ 該重送的不送）。兩個方向都實際發生過。廣播只由 presence 判定與解除；瞄準的那種才比對機器。`notifyWorkerSpawn` 解除舊 kill 時問的是 `owes(那台機器)`，不是 `== Target`。
...
- 🔴 **關機那一步不可以在 `outsourceMu` 裡跑**：`dispatchShutdown` 自己會依序取 `outsourceMu` 再取 `reconcileMu`，照 T-14 owner ruling 的 lock A → run A → drop → lock B → run B → drop。外包 report_stopped 因此是「上鎖決策＋落 latch → 放鎖 → 關機 → 再上鎖記帳」。巢狀持有會當場 deadlock（Go mutex 不可重入），而 `TestDispatchShutdown` 裡那支鎖序測試是唯一會指名這件事的守衛。
