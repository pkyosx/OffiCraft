...
- 乾淨 worktree 手跑 server Go tests 前，先在 repo 根目錄執行 `make build-embed-assets`（`make test-go` 與 `bin/ci.sh` 會自動執行）。要 staging 哪些東西以 Makefile 的 `build-embed-assets` 為準，不另外列在 server/CLAUDE.md。只跑單一支測試也要先執行；少了這一步會出現看起來像程式壞掉的假紅（`open mcp-catalog.json: file does not exist`、`ocwarden binary is not available`）。
...
- 🔴 **新增 migration 的號碼必須大於 `origin/main` 目前宣告的最大號**，兩個來源都算（`migrations/*.sql` 與 `goose.AddNamedMigration*` 註冊的 Go migration）。跳過的號碼永遠留著不補：低於已釋出最大號的新號會讓**已經在跑的站**下次開機在 `runMigrations` 停住、exit 1、不會 listen，而全新安裝完全看不出來（空白 DB 上「缺號」不存在）。**同樣不可逆的另一半：已經釋出的 migration 檔案內容不可以再改**——goose 一個版號只記一次、永不重跑，改動只會落在全新安裝上，兩種站從此 schema 不同而且沒有任何錯誤。要改就開新號。**這一半兩個來源都會紅**：`migrations/*.sql` 比對整個檔案，Go migration 比對它**整份原始碼檔**（不只 up/down 函式主體）——所以一個純機械的改動（改名波及、gofmt）也會叫，那時要由人決定怎麼辦，而不是由這道檢查默默放行。
  ⚠️ **「大於 main 的最大號」是必要條件，不是充分條件**：兩條同時在飛的分支各自對著同一顆 main 取 max+1 會取到同一個號。取號前仍要掃所有遠端分支已占用的號——**但那只是省事，不是防線**：任何靠「掃遠端」的做法都要等兩邊會合之後才看得見，而那時已經在主線上了。防線是 `server/ocserverd/migration.lock`。
  🔴 **`migration.lock` 的工作就是把一次乾淨的 merge 變成衝突。** 它是一份**不分區的扁平清單**（`migrations/*.sql` 與 Go migration 混在同一份、照版號升冪、只在尾端 append），第一行是整份清單內容的 `roll sha256:…`，其餘每行是 `NNNNN 路徑 sha256:<該支內容的雜湊>`。兩條各自帶 migration 的分支都會動到尾端與 `roll` ⇒ git **一定**在這個檔上衝突，**包含號碼其實沒撞的情況**——那是刻意付的代價，不是 bug。（分區寫法、或標頭只放 `count=`／`max=` 這種摘要行，實測都會被 git 靜默 auto-merge 掉：撞號時兩邊會寫出**同一個**摘要值，而 git 對「兩邊做了一模一樣的改動」不報衝突。量測與理由見 `server/ocserverd/migration_lock.go` 檔頭。）
  🔴 **但衝突不是唯一的機制，而少了衝突的那一格更容易漏掉。** 一條**比 lock 還早存在**的分支從來沒有這個檔 ⇒ 合主線時 git 把它當**新增檔案**直接收下，**乾淨、沒有衝突、沒有任何東西逼你停下來看一眼**——而這時樹上有你的新 migration，剛收下的那份清單卻沒有列它。⇒ 擋你的變成 `TestMigrationLockMatchesTheTree` 的 `[lock:missing]`，`go-checks` 紅（必要檢查），合併鍵一樣按不下去，只是你要多跑一輪 CI 才知道。
  ⇒ **所以把「合完主線就跑一次 `./bin/gen-migration-lock`」當成無條件動作，不要等衝突提醒你。** 三種情況只有一種會有衝突：

```
  兩邊都已經有 lock、都往檔尾 append   → git 衝突，合併鍵鎖住（實測 GitHub 回 CONFLICTING/DIRTY）
  一邊第一次遇到 lock、沒重新產生      → 不衝突，但 PR 的 go-checks 紅（[lock:missing]）
  一邊第一次遇到 lock、有重新產生      → 乾淨，正確
```
  ⚠️ 順帶一個判別法：**有衝突的 PR 一格 check 都不會有**（實測 `check-runs total_count = 0`，不衝突的對照組 11）。看到「沒有任何 check」不是 CI 壞了，是它還在衝突狀態——而**沒有結論的 check 不是 pass**。

  **衝突了怎麼解**：不要用手接文字。先確認兩邊的版號真的沒撞（真的撞了就把後進來的那支 migration **檔案本身**改名到 max+1），再跑 `./bin/gen-migration-lock`——它從工作樹重新列舉並整份重寫，`roll` 是算出來的，**任何情況下都不得手改**。改完看 diff：新 migration 只能是**尾端新增的行**；**中段有行被改動 ＝ 有一支已釋出的 migration 被編輯或刪除了**，那時要停下來查，不是繼續 commit。
  ⚠️ **重新產生 lock 不等於那次改動被允許。** 編輯一支已釋出的 migration 之後跑產生器，`TestMigrationLockMatchesTheTree` 會變綠——因為 lock 現在**誠實地描述了這棵樹**（實測：改 `00022` 一行註解 ⇒ 未重新產生時紅在 `[lock:content]`，重新產生後 `ok`，而 lock 中段第 42 行的雜湊變了）。說「不准」的是拿本樹跟 `origin/main` 比的那半（`TestMigrationLockGrowsOnlyAtItsTail`，要求 main 的行是本樹的**精確前綴**）。🔴 **它在 main 分支上是空轉的**（本樹就是 main，沒有東西可比）——**它只在 PR 路徑上活著**，而中間那一段唯一的守衛是**讀 diff 的那個人**。
  上面幾條由 `TestMigrationLockGrowsOnlyAtItsTail` 釘住：它要求 main 的 lock 行是本樹 lock 行的精確前綴。（T-75 之前還有一支 `TestAStationAtTheReleasedVersionCanUpgradeToThisTree` 重播站台升級路徑，已在實測「停用它之後撞號／改已出貨內容／改名／刪除都仍被 lock 擋住」後拆除。）**它讀的是本機的 `origin/main` ref、不會自己 fetch**，所以本機跑之前先 `git fetch origin main`——🔴 **ref 過期會把它扭向兩個方向，不是一個**（T-64 `be10ac8d` 逐字更正過同一句，這裡不要再寫回單向版本）：**太寬鬆那一邊是靜默的**（main 較新的那幾行根本不在基準裡，前綴比對是拿一份比較短的清單在過）；**太嚴格那一邊會出聲**——main 自己在那顆 ref 之後改寫過的每一行，都會被讀成**本樹**動了一支已出貨的 migration，而那份報告指名到行、指名到 sha，具體到足以讓人相信。⇒ **看到紅先看基準的日期**（那一行的 `t.Logf` 會印出來）。解析不到 `origin/main` 時本機 skip 並說明（**skip 不是綠**），在 CI 則是直接紅，因為 CI 沒有基線就等於這道檢查被關掉。CI 那一側靠 `go-checks` job 的 `actions/checkout` 帶 `fetch-depth: 0` 才有那個 ref，**那一行是承重的**。

## 2. wire、route 與權限是同一個契約

- `spec/openapi.json` 先於 handler：它是 wire 的唯一來源，也是手改入口，`bin/gen-ocapi` 由它產生 `ocapi_gen.go`；`ocapi_gen.go` 是生成物，手改會被 `drift-ocapi` 擋下。route table 在 `routes.go`，用 `Public(routeDef{...})` 與 `Gated(class, routeDef{...})` 登記，`routeDef` 沒有 Auth 與 Requires 欄位——權限漏寫或寫錯是編譯錯誤，不是執行期檢查。新增 endpoint 必須讓來源檔、route table 與 `conformance/routes_manifest.json` 同批一致，不能只改 handler；conformance 的 `test_openapi_covers_manifest` 把 spec 與 manifest 釘成對稱相等，漏登記或殘留會在那裡當場失敗。⚠️ 這道保證跨語言：本機只跑 Go 側不涵蓋它，要連 conformance 一起跑才完整（CI 的 `bin/ci.sh` 兩者都跑）。比較是集合對集合，所以**看不見 `routes_manifest.json` 內部的重複列**——重複只會讓同一條 route 被多測一次，不會讓任何 route 逃掉權限測試。
...
- `reconcileDecide` 是 desired × observed 的純決策；cadence 是 sleep-then-tick，`--no-reconcile` 才停用 producer。member 與 worker **共用同一支 reconcile 狀態機與同一組 refocus／stop／start 流程** —— owner 2026-08-17（`c-8c1cf57cd9f4`）與 2026-08-21（`c-aca1d5116a47`）逐字裁定「重新聚焦這個流程兩邊應該要是一樣的，下線上線也是」「正職跟外包不應該在 refocus / change model or machine / stop / start 有任何差異的必要」。仍然**不要**為了表面對稱而合併的只剩兩樣：它們的**觀測輸入**，與 worker 專屬的 **task state**。relocation 也仍然刻意留在 worker 自己的 mask 裡，因為它牽動 machine placement 那一整套 pin／stall 規則。
- 🔴 **全 package 只有一個地方會建 `stop` frame 並交給 warden**：`shutdown.go` 的 `enqueueStopFrames`（外加它的 receipt-watch 包裝 `sendStopFrames`）。報停收口、外包 tick（換手／停止／殭屍接管／reclaim）、正職 tick（desired-offline 停止與 relocate STOP）四條路都走它，各自只留自己的記帳（park／retry 標記、lifecycle phase、`DispatchUnlanded`、sweep 去重）。它**不取任何 scheduler lock**，這正是已持鎖的兩個 tick 能直接呼叫的原因——在裡面伸手去拿 `reconcileMu`／`outsourceMu` 會一次製造三條巢狀持鎖路徑。刻意不走它的只有兩個，都在碼裡寫明理由：identity sweep（走不 arm watch 的那一半）與 UNINSTALL（不是 stop 動詞，且刻意不 watch）。
- 報停路徑再往外包一層是 `dispatchShutdown`，正職與外包同一份：解析目標機器 → 送出（上面那支）→ 清 spawn 節流 → 清 session boot ts → 記下 at-least-once dispatch marker。兩邊唯一還分岔的是**解析目標機器時可用的來源**（外包多一個 in-memory `workerSpawnTarget`），那條 kind 判斷在 identity-gate ledger 上有登記與理由。**kill chain 的順序是可執行權威，不在這裡抄**：讀 `killTargetChain` 的註解。要記得的只有兩件事——`last_machine_id` 在正職這一側排在 desired pin **之前**（搬機器後 pin 已指向新機器，舊 session 還在原機），以及鏈尾是**廣播給所有在線 warden**（指令以 session 名定址，沒有那個 session 的機器是 no-op）。⚠️ **這只是「關機找機器」**：外包開機的 `desired_machine_id` 仍然是 hard pin、不可用就 STALL，兩者不要互相套用。
- 🔴 **一道 kill 的「有沒有被記住」是同一個判準，兩條路都要讀它**（`workerStopOutcome.recorded()`）：tick 的換手與報停的收口共用它。報停那側曾經改用「有沒有指名過目標」，那是嚴格較弱的問題，於是「廣播且全被拒」整個溜過去——沒送、沒記、沒回滾，wire 還回 `collected`。⚠️ 這個形狀**同步呼叫內造不出來**（`onlineWardens` 與 `enqueueToWarden` 讀同一個 `hub.IsOnline`），只能由機器在兩次檢查之間掉線產生，所以它的測試打在 `concludeWorkerStoppedReport` 這個接縫上，不在 endpoint。
- 🔴 **kind 判不出來時 fail-closed 到「外包」**：那個旗標決定要不要裝正職的 `RobustStopPendingAt`，猜錯成正職就是上面那個病本身；猜錯成外包只讓一次正職關機少一道 cadence 重送。讀不到 roster 時要落在便宜的那個錯上。
- 🔴 **`reconcileState` 是兩個 population 共用的一份 store，所以「只屬於一邊的標記」要在寫入端就擋住**：`RobustStopPendingAt` 是**正職** producer 的 at-least-once 臂，`dispatchShutdown` 只對非 outsource 的 subject 寫它。寫到 worker 身上會被外包 tick 的 `reconcileDecide` 讀到——先用 `robustStopWait` 壓住該發的 START，過了 `StopRetry` 再變成一道 STOP，而 worker 的 STOP 分支會把它當殭屍接管、**把機器 bench 掉**。外包自己的 at-least-once 是 `workerStopLanded`／`workerStopPending`，兩套不可以同時上。⚠️ `reconcileWorkerLiveness` 的 bench 只給 `zombie_takeover`，那道 guard 是**活的**（T-253 讓它重新可達）；它曾被當 dead code 拿掉，理由寫「robust_resend 需要 workerObservation 不填的欄位」——那句話本身就是錯的，marker 在 lifecycle state 裡，不在 observation 裡。
- 🔴 **一道 kill 送完要問的是「它被記在某處了嗎」，不是「它送出去了嗎」**：落在某台 warden 的 FIFO 上、或被 park 在某台具名機器的欠帳裡，都算有記；**廣播全被拒＝兩者皆無**（fan-out 無從 park——park 的意思就是「這台還欠一刀」，而廣播的前提正是沒有任何來源說得出機器）。這一格必須讓呼叫端 defer 並回滾 latch，不可以當成成功——它是這條路上唯一「殺不掉又沒人記得」的形狀，而且失敗時完全無聲。`stopWorkerSessionForHandover` 只留**一支** defer 臂問這個問題（「沒目標」與「全被拒」是同一件事）；`stopWorkerNow`（held-down）照 F4 不 defer，只出聲。
- 🔴 **一道 kill 要嘛「瞄準了某台」要嘛「廣播出去」，而廣播不可以假裝自己瞄準了誰**（`workerStopDispatch.Target`，廣播是 `""`）。廣播的前提就是沒有任何來源說得出機器，所以它打到的每一台都可能從沒 host 過那個 session：把其中任一台塞進 `Target`，那台一句例行 `no_such_session` 就解除整個重送（＝ 殘活 session）。三個讀者因此各自分臂——收據**完全不解除**廣播、重送用**純 presence** 判定、`resendKillTargets` **重新解析 chain**（不重播舊名單）。
- 🔴 **廣播的臂由「任何一次 landed START」解除，瞄準的臂才只由它自己那台解除。** 這條是用殺錯 session 換來的：廣播打到 [A,B]、C 當時是暗的、C 回線後替補起在 C，若解除條件只認 A/B，臂就留著；過了 StopRetry worker 讀成在線，重送重新解析 chain——而 chain 此刻指向 C，因為 live claim 已經是**替補的**。刀就落在剛建立的 session 上。廣播沒有立場宣稱「某台還欠我一條命」，所以任何 START 落地都解除它。
- 🔴 **收不到手的關機，兩個 population／兩種 desired_state 的收尾不一樣，而那個不一樣是刻意的**：整個 fleet 都連不上時（chain 連廣播都空手），**還要回來**的成員要回滾 `stopped_since` latch 並在列上說明，**被 owner 壓著不准起來**的（`desired_state=offline`）則不回滾、不寫 receipt——回滾會把一個真的完成的收尾變成沒完成，而 receipt 會對一個沒人打算重試的 worker 說「retrying」。同一個理由：什麼都沒送出去時**不要清 `session_boot_ts`**，沒有 session 死掉，清了會讓 `restart_self` 的最低存活檢查與 boot-storm guard 對一個還活著的 session fail open。
- 🔴 **放掉鎖再回來，就不可以再用放鎖前的那份 row 快照做決定或寫回**（外包報停是唯一有這個空窗的路徑）：重讀一次，而且回滾只寫該欄位（`SetMemberWindDownAnchors`），不要整列寫回——整列寫回會把空窗期間別的請求改過的每一欄悄悄還原。
- 🔴 **關機那一步不可以在 `outsourceMu` 裡跑**，但**兩個 population 靠的東西不一樣，說法不要混**：正職的 `dispatchShutdown` 真的會依序取 `outsourceMu` 再取 `reconcileMu`（T-14 的 lock A → drop → lock B），那條順序由 `TestDispatchShutdown` 的鎖序測試量；**外包自 F1 起根本不碰 `reconcileMu`**，所以「不可同時持有」在它那側是**空真**，真正擋住它的是 `dispatchShutdown` 會**重入 `outsourceMu`**——沒放鎖就當場自我 deadlock（Go mutex 不可重入），實測會卡在 `resolveShutdownTargets`。外包 report_stopped 因此是「上鎖決策＋落 latch → 放鎖 → 關機 → 再上鎖記帳」，而它的測試屏障是 `receiptMu`，不是 `reconcileMu`。
  ⚠️ **但不要把「有專責守衛」讀成「鎖用錯不會再變成整包 timeout」。** 那句話只對**正職**成立（它的守衛以持鎖為屏障，該紅就紅）。外包那側把鎖留著不放時，專責守衛確實會在幾秒內以具名訊息失敗，可是**同一輪還有數十支測試各自同步呼叫 `/api/self/stopped`**，每一支都會在 `resolveShutdownTargets` 當場自我 deadlock，於是整包照樣走到 timeout panic 並帶走所有結果——實測如此。沒有任何守衛設計救得了這一格：它是「每個呼叫者都會卡住」，不是「某一支斷言沒過」。看到整包 timeout 又剛好動過這條路的鎖，先懷疑這個，不要以為那種事故已經不可能重演。
- 🔴 **report_stopped 的兩步 durable 寫入（anchor、然後整列）失敗時要把 `stopped_since` latch 回滾**，正職外包都一樣。不回滾的話第二步一失敗就永久失效：重試會讀到 prior>0、回 `already_reported`、一道關機指令都不再送，而那一列還宣稱收尾完成。成功路徑不回滾，所以正常的重複回報仍然是 `already_reported`。
...
- session boot anchor `member.session_boot_ts` 是 durable、不上 wire。reconnect 從持久值恢復 gauge；每一條成功 enqueue 的新 session start 與 stop boundary 都要清 anchor，使用 `SetMemberSessionBootTS` 單欄更新，不用整列 `PutMember` 代替。start 端走 `clearSessionBootTSForStart`：它先記下被清掉的 session 狀態，warden 以 `session_already_exists` 拒絕那次 start（舊 session 還活著）時由 receipt fold 還原；其他 start receipt 與任何 stop boundary 都丟掉這份記憶。已知邊界：舊 snapshot 的整列 upsert 仍可能復活舊 anchor；若要改變該資料競態，另行取得 owner 裁定。
...
- 🔴 讀的那一側（`authz.go agentIatFloorRefusal`）**明確把 `Kind == machineKind` 排除**，寫的那一側則不分 kind。這是安全性質不是最佳化：warden 憑證是 `scope="agent"`，而底線一旦蓋過它，這台機器要等到自己換發成功才回得來——換不成就只能重裝。⚠️ **理由在 T-fc53 第二段收窄了**：原本寫的是「沒有 `exp`，所以永遠過不了期」，那在第二段之前是對的；現在憑證有 `exp`（預設 30 天），所以「永遠」不再成立，但豁免仍然要留著——被底線擋住的 warden 連換發端點都打不進去，於是它會卡到憑證自然到期為止，結局一樣是重裝。warden 今天不呼叫 `report_waking`，但那是今天的 client 而不是契約。由 `TestAgentIatFloor_WardenPermanentTokenIsExempt` 守住，不是靠註解。
- ⚠️ **上面兩欄不是「整列寫入不得帶」的全部，這份文件也不打算列全。** 那份清單會長，逐一寫在這裡就是一份會靜默過期的固定列舉（它已經過期過一次：`banked_cost` 搬走時沒有人回來加一行）。**權威是可執行的**：`single_column_writes_t14_test.go` 的 `singleColumnOwnedFields` 註冊表就是那份清單，`TestPutMemberNeverOverwritesSingleColumnOwnedFields` 會在任何一欄的 `insertOnly` 被拿掉時**當場紅並指名該欄**（`TestMemberColumnPropertiesAreDeclaredInOnePlace` 從另一頭釘住同一件事：它比對整份分類，欄位改變身分就紅）。要知道「今天有哪幾欄被搬走了」讀那張表，不要讀這裡。上面兩欄留在這裡是因為它們各自帶著**只有散文講得清楚的理由**（warden 憑證的 iat 底線豁免、`memberFromWorker` 從零重建會送 0），不是因為它們是清單。
...
- 文件 history list 只回 metadata；body 用指定 kind + key 的 `get_document_version` 逐筆取，seed 用 `get_document_seed`。role definition、task manual SOP 的 restore 需要文件仍存在。SOP 自己一條有 cap/history 的序列，purpose、display、assignee 不在 version body 裡。
- context cap 的三個 key 是 duty、insight、manual SOP，各自 accessor、預設值到上限 100000；`global_context` 與 `task.description` 不套這組 cap。長度單位是 Unicode rune，不是 byte。
- 受 cap 的寫面都遵守同一條：新內容未超過下限可寫；超過下限時必須嚴格短於舊內容；等長或變長拒絕。比較 caller-visible 的 folded content（含 seed/overlay），`allow_shrink` 是另一個 wipe guard，不是 cap bypass。回應要同時報 size 與 cap。
- Duty、Insight 是不同文件，不自動互搬；Insight key 只有 `role_key`，沒有 task type。agent 編輯 Insight 前讀 server role docs 與 per-role seed；無 role 的 caller 沒有自己的 Insight capability。
