...
func routeSpecs(w *ServerInterfaceWrapper) []RouteSpec {
	rows := []routeRow{
		...
		Gated(principalAdminAgent, routeDef{
			...
			Summary: "Last 5 raw /in requests of one webhook endpoint, newest first (debug). /in answers 200 {\"status\":\"ok\"} whether or not anything was delivered (only a Slack URL-verification handshake gets its challenge echoed instead), so this log is the only place the outcome shows: ``delivered``, ``challenge``, ``ping`` (a verified GitHub ping, not delivered), or ``dropped:<reason>`` with reason ``disabled``, ``sig_failed`` or ``member_gone``. A request with a missing or unknown token matches no endpoint and is logged nowhere.",
		...
		Gated(principalMachine, routeDef{
			...
			Summary: "Post a chat message (sender = verified JWT sub; auto SSE fan-out). ``to`` must name the owner or an active staff or outsource member; an unknown id, a removed member, a warden (machine) id and a synthetic sender such as ``hook:<endpoint_id>`` are all a 404. Presence is not a gate: an offline member keeps its durable mailbox. Answers with a bounded receipt (``id``, ``ts``, ``to``, ``attachments``), not the message — call ``get_chat`` when you need the rest.",
		}),
		Gated(principalMachine, routeDef{
			Method:  "GET",
			Path:    "/api/chat",
		...
		Gated(principalAgent, routeDef{
			...
			Summary: "Move one 傳承 entry to active / pinned / retired. 置頂 and un-置頂 are ADMIN-ONLY (owner ruling): a pinned entry sorts ahead of every other entry in its scope and so survives the cap at the others' expense. 失效 and 生效 are open to the entry's own AUTHOR -- anyone else is a 403 -- and admin capability is unrestricted. A PINNED entry is outside its author's reach as well: until an admin un-pins it, every move a non-admin requests on it, the author's included, is a 403. Retiring is not deleting: the entry keeps its id and can be moved back; ``retire_reason`` is stored only with retired and cleared by the other two.",
