...
func routeSpecs(w *ServerInterfaceWrapper) []RouteSpec {
	return []RouteSpec{
		...
		},
		{
			...
			Auth:     authGated,
			Requires: principalAgent,
			Summary:  "Submit/replace the workflow plan (done and answered-card steps are kept). \u26a0\ufe0f REPLANNING DELETES EVERY UNFINISHED STEP AND THE WORKING NOTE ON IT, and there is no way back \u2014 the note lives on the step row, the row is deleted outright, and no version history keeps a copy (measured 2026-09-08 on a throwaway database: a pending step and an in_progress step both vanished with their notes; a 1,811-character hand-off note has already been lost this way once). WHAT SURVIVES, note and id both: steps that are done, steps already superseded, and steps carrying an ANSWERED reply card (those are frozen rather than dropped). EVERYTHING ELSE IS REBUILT, including a step you relist under the SAME NAME: it comes back as a NEW step with a NEW id and an EMPTY note, and the old id 404s from then on \u2014 so a hand-off note is not saved by naming the step the same thing. Copy anything you need to keep somewhere else BEFORE you resubmit. T-74f8 \u4ea4\u68d2\u9598 (second door): a plan is a step-set write and the task status is DERIVED from the step set, so a plan that leaves EVERY step done CLOSES the task — the same irreversible close the final step report performs. If that task's creator is not its executor and no handover is declared or already real, the replan is refused with 422 BEFORE anything is written (the plan stays fully editable). A plan carries no handoff field, so the way out is to hand over first: create the successor task and point its ``blocked_by`` at this task (the gate then stands aside by itself), or keep one unfinished step and declare the handover on the ``update_step_status`` report that closes it. A replan that still leaves work in the plan is never gated. Answers with a bounded receipt (task_id, steps_total, progress_done, progress_total), not the plan you just sent — use get_task to read the stored step rows back.",
			MCPTool:  "submit_plan",
		},
