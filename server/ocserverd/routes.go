...
func routeSpecs(w *ServerInterfaceWrapper) []RouteSpec {
	rows := []routeRow{
		...
		Gated(principalAdminAgent, routeDef{
			...
			Summary: "Last 5 raw /in requests of one webhook endpoint, newest first (debug; owner or admin agent only). /in answers 200 {\"status\":\"ok\"} whether or not anything was delivered (a Slack URL-verification handshake gets its challenge echoed instead, and a server-side database failure is a 500), so the caller of /in cannot see the outcome; the endpoint row keeps only counts and the last drop reason, and this log shows the outcome of each request it records: ``delivered``, ``challenge``, ``ping`` (a verified GitHub ping, not delivered), or ``dropped:<reason>`` with reason ``disabled``, ``sig_failed`` or ``member_gone``. A payload over 1 MiB is cut to its first 1 MiB and, if delivered, still logs ``delivered``. A request with a missing or unknown token matches no endpoint and is logged nowhere. A request that ends in that 500 is not logged either, and a log write that fails is skipped without an error, so a request can be missing from this log.",
		...
		Gated(principalMachine, routeDef{
			...
			Summary: "Post a chat message (sender = verified JWT sub; auto SSE fan-out). ``to`` must name the owner or an active staff or outsource member; an unknown id, a removed member, a warden (machine) id and a server-generated sender address (``hook:<endpoint_id>``, ``sched:<schedule_id>`` or ``system``) are all a 404. Presence is not a gate: an offline member keeps its durable mailbox. Answers with a bounded receipt (``id``, ``ts``, ``to``, ``attachments``), not the message — call ``get_chat`` when you need the rest.",
		}),
		Gated(principalMachine, routeDef{
			Method:  "GET",
			Path:    "/api/chat",
		...
		Gated(principalAgent, routeDef{
			...
			Summary: "Write ONE 傳承 entry (never editable afterwards). ``task_id`` picks the scope, and there is ALWAYS somewhere for it to land: a task that carries a TYPE files under that type's manual; no task at all, OR a task with no type (臨時任務), files into your OWN boot document, under your own member id (scope ``agent``), staff and outsource members alike. The untyped-task case answers a ``scope_note`` saying where it actually went, because you asked for a manual and did not get one. Only a caller with no roster row at all is a 400 -- there is no boot document to file into. Title and body are capped in characters (Unicode code points) by the settings ``lore.cap_chars.title`` and ``lore.cap_chars.body``, 80 and 500 unless the owner or an admin agent has changed them; an over-cap title or body is a 400 that writes nothing and states the cap in force.",
		...
		Gated(principalAgent, routeDef{
			...
			Summary: "List 傳承 entries, filtered SERVER-SIDE and paged in the fixed order pinned -> active -> retired, newest first inside each group. The order is not configurable; the filter is. Nothing is narrowed by who is calling: with no author or scope filter, every caller sees the entries of every member and every manual.",
