...
func routeSpecs(w *ServerInterfaceWrapper) []RouteSpec {
	return []RouteSpec{
		...
		// history rather than the feature.
		{
			...
			Auth:     authGated,
			Requires: principalAdminAgent,
			// 🔴 THE OPT-OUT LIVES IN TWO PLACES AND BOTH HAVE TO SAY IT. The
			// x-mcp `include: false` above in spec/openapi.json is only half:
			// mcpToolIndex filters on THIS field, so a row that carries the spec
			// half and not this one is still a callable tool. That is exactly how
			// this route shipped — the comment above argued the exclusion, the
			// spec recorded it, and the tool surface kept it anyway, which is
			// what TestMcpToolIndexMatchesFrozenCatalog caught (index 140 vs
			// catalog 139). Neither half is redundant and neither is the source
			// of truth; the tests exist because they can disagree silently.
			MCPExclude: true, // 餵座艙的傳承活動面板；今天沒有任何 agent 使用情境。要開放給 agent 之前先有具體用途（Kyle 2026-09-06 裁定）
		...
		// 沒有任何東西能分辨一格是真的填出來的還是硬掰的。審核仍然發生在寫入之後。
		{
			...
			Auth:      authGated,
			Requires:  principalAgent,
			Summary:   "Write ONE lore entry — 標題格與內容格加上 `events`, the subjects it is filed under, and the FULL ORIGINAL that outlives every later rewrite, all in one transaction. The cells are `heading` (標題 — 發生了什麼，也是讀者找到它的那一軸), `content` (內容) and `events` (相關的完整資訊 — 0..N 筆 時／事／人／地／物). ⚠️ THERE ARE NO `revisit_when` / `impact` / `impact_stars` / `supersedes` CELLS ANY MORE: owner ruled on 2026-09-06 (message c-3d3e5582c2d2), verbatim 「都改掉」, that the entry shrinks to an MVP, and all four went with their database columns. `reviewed` — a flag that used to say somebody stamped the 星等 — is NOT writable here and now has nothing left to stamp; it stays only because the owner did not name it. 🔴 `heading` and `content` are the two that are REQUIRED: `content` is the only cell that ever enters a boot context, and `heading` is BOTH the line a human reads in a list AND the axis a reader finds the entry by — an entry missing either is not thin, it is either unreachable or indistinguishable from a finished one. ⚠️ THERE IS NO `trigger` CELL ANY MORE. v8 first pulled the 標題 out of it into its own `heading`; then owner ruling rc-9002654dd81c (2026-09-06), verbatim 「合併成 heading 一格（同時把搜尋改成掃 heading＋內容、待審畫面改顯示 heading）」 merged the two back into one. The split was making the same memory introduce itself with a different sentence on each screen — the list showed `heading`, the review queue showed `trigger`, and `query` scanned `trigger` only, so the one line a user could actually see was the one line search could not find. 🔴 `heading` IS CAPPED AT 140 CHARACTERS and the cap is in RUNES, not bytes — a Chinese character counts as 1 (owner ruling 2026-09-05, verbatim: 「我們標題規定 140 字元好了」). Over the cap the WHOLE write is refused 422, naming the cell, the cap and the length you sent; it is NEVER truncated, and no entry already stored is touched. The same cap closes the proposal route — filing one is refused, and so is accepting one, because accepting writes the 標題 back onto the entry. Measured 2026-09-05: the longest of the 24 headings rewritten to the v8 format is 130 runes, so nothing on the station is refused today and the longest one sits 10 runes under the cap. ⚠️ Owner ruling rc-714eea33c6ed (2026-09-02), which made `falsify` and `instance` purely required, has NO LANDING PLACE in this format — neither cell exists any more. It was not overturned; the format change left it with no field to apply to, and whether 五格 should carry a cell meaning either of those things is the owner's call. `events` is `events`: every event needs `happened_ts` (when it HAPPENED, not when it was written down) and `what` (active voice, so the 人 is always the one doing it), while `actor` / `place` / `object` are sent only when you actually know them and are NEVER back-filled with 「未知」 — 「查不出是誰」 and 「還沒有人去查」 must not end up looking the same. Zero events is legal, and a bad event refuses the WHOLE write rather than leaving an entry half-written. `subjects` are subject keys shaped `type:name` (`repo:officraft`, `agent:Kyle`): an alias resolves, a merged-away subject follows to the survivor, an unapproved type prefix is refused BY NAME, and a key nobody has used yet MINTS a new subject parked for review and names it back to you in `pending_entities` — so a typo surfaces in this response instead of in the ontology a month later. ⚠️ There is NO `origin` field any more. It named WHOSE knowledge an entry was (`human:Seth`, `agent:Kyle`) and bought that entry a priority: it sorted ahead, ignored `limit`, and reserved its subject against boot-directory truncation. Owner removed the field and all three mechanisms on 2026-09-06 (rc-9c9bf14a579f, verbatim 「A｜對，一起拿掉。我知道我說過的話從此跟其他條目一起搶額度」). Nothing replaced it: who wrote an entry is now only the actor from your verified token, and what a human said competes for room like everything else. ⚠️ There is NO `supersedes` field any more, and nothing replaced it: a new entry cannot declare which one it takes over from, and the entry it replaces is NOT re-statused — the two sit side by side and reads return both. ⚠️ There is NO `degraded` flag on the receipt any more: owner ruling rc-1e32c690018d (2026-09-03) removed it, because 標題格 is already a hard refusal at the door and a second, softer quality mark behind it earns nothing.",
		...
		// people look: `content` and the events are NEITHER searched NOR returned
		// on a hit — a hit carries `heading` and `subjects`, and the rest is read
		// with `get_lore_entry`. There is no table, no index and no parameter for
		// them here, so neither is reachable through this route today. That is a
		// known gap, not an oversight.
		//
		// ⚠️ This paragraph used to name `symptoms`, then `impact`, as the cell
		// de-duplication and conflict-finding would have to run on. **Neither cell
		// exists any more** (`symptoms` went in the six→five change, `impact` in
		// owner 2026-09-06「都改掉」)。The gap did not go with them: it has nowhere
		// left to sit, and nothing today answers 「這兩條在講同一件事嗎」.
		{
			Method:    "POST",
