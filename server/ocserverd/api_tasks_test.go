	...
	t.Run("handing a task to another member answers the receipt under the reassigning lock and pairs both sides in chat", func(t *testing.T) {
		...
		apiWantValue(t, "chat", apiTestChatRows(t, d), []any{
			map[string]any{
				"from": "system",
				"to":   "kip",
				"body": "[T-1] 此任務已轉派給新的接手人。你收到這份說明，代表目前的任務需要交接給其他執行者。請停止推進並完成必要收尾，確保接手人能從遠端取得目前成果與完整脈絡：\n" +
					"\n" +
		...
		dashboard.wantFrames(
			...
			map[string]any{
				...
				"data": map[string]any{
					...
					"epoch":   2,
					"deleted": false,
					"payload": map[string]any{"id": apiAnyString, "name": "X-1", "status": "active", "desired_state": "online", "owner_id": "owner"},
				},
				"ts":      apiAnyNumber,
				"trigger": "server",
			},
