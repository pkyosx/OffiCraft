...
func TestNewOutsourceWorkerDTO_GoldenWireShape(t *testing.T) {
	...
	fullProjection := outsourceWorkerProjection{
		...
		// A NAMESPACED station's command (T-139). The golden must show a
		// socket no derivation on this side could invent, or a builder that
		// re-derived it from the id would reproduce the golden exactly.
		terminalAttach: "tmux -L officraft-seth attach -t member-ow-1",
	}
	cases := []struct {
		name string
		w    OutsourceWorker
		...
		},
		{
			...
			// terminal_attach_command is "" because this projection carries
			...
			want: `{"id":"ow-2","avatar_icon_id":null,"codename":"O-8","runtime":"claude","model":"claude-haiku-4-5","effort":"","actual_model":"","actual_runtime":"","actual_effort":"","status":"assigned","task_id":"t-2","task_title":"","task_status":"","task_no":"","task_created_ts":0,"task_type_key":"","task_type_name":"","created_ts":1999,"unread_count":0,"presence":"offline","machine":"","desired_machine_id":"","actual_machine":"","account":null,"context_pct":null,"cost":null,"banked_cost":null,"last_op":"","last_op_ok":null,"last_op_log":"","last_op_reason":"","last_op_at":0,"creator_id":"","delegated_by":"","refocus_since":0,"refocus_op":"","refocus_deadline":0,"desired_state":"","terminal_attach_command":""}`,
		},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
...
func TestFoldActorRuntime_Mainline(t *testing.T) {
