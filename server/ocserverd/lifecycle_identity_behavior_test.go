...
var identityGateLedger = map[string]string{
	...
	"api_tasks.go :: HandleReassignTaskApiTasksTaskIdReassignPost :: leavingKind == TaskExecutorOutsource": "" +
		...
		"reap it. A bound outsource worker exists only for this task and is dismissed " +
		"by its own id; a staff member lives on its own lifecycle and must not be " +
		"(dismissOutsourceWorkerByID would also expire every card it opened anywhere).",
