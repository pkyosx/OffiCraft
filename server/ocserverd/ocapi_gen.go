...
type LoreEntryDetailDTO struct {
	...
	// Heading 標題格 — v8 的獨立一格：這條條目在講的是「發生了什麼」。⚠️ 空字串是「還沒寫」，而它幾乎只會出現在 v8 之前寫下的條目上 —— 那時候沒有這一格，`trigger` 兼任標題，而 migration 對既有列只給得出一個空的預設值。「作者留白」與「這條比這一格還老」在這裡分不開，因為沒有任何欄位記得下來。
...
type LoreEntryRefDTO struct {
	...
	// Trigger 第 1 格 — 「什麼時候要記起來」. ⚠️ It USED TO double as the entry's title and since v8 it does not: 標題 is its own cell, `heading`, and that one is capped at 140 characters (Unicode runes) while `trigger` still has no length cap. This row carries `trigger` because it is the cheapest thing that answers 「what is filed under this name」 — it was designed to be read alone.
	Trigger string `json:"trigger"`
}
...
type LoreProposeDTO struct {
	...
	// Impact The proposed 第 4 格「impact」（原本想達成什麼、實際變成什麼）. Optional, exactly as on a write. Sent only on an `update`, where the four together with `events` are the whole new version; on a `remove` it must be absent or empty.
...
// LoreWriteDTO Create one lore entry: 六格 (a heading, four body cells plus 0..N events) and the axes it is filed under. The field set is CLOSED — an unknown key is a 422, never a silent drop.
type LoreWriteDTO struct {
	...
	// Trigger REQUIRED. 第 1 格「什麼時候要記起來」— the situation you would be IN when this entry applies, written as the sentence you could have written BEFORE you knew the answer. It is the axis a reader finds the entry by, and since v8 it is NOT the entry's title: 標題 is its own cell, `heading`, and THAT one is capped at 140 characters (Unicode runes). `trigger` itself has no separate `label` field and no length cap, because the owner's own worked example writes the first cell as a full sentence far longer than the 40-rune cap `label` used to carry. Blank is REFUSED rather than defaulted — an entry without it sits in the table looking exactly like a finished one and nobody can retrieve it. ⚠️ 「第 1 格兼任標題，因此沒有 `label`、沒有長度上限」是實作判斷，不是負責人的裁定 —— 它的前半（兼任標題）已經被 v8 推翻，後半（`trigger` 這一格沒有長度上限）到今天仍然成立。有上限的是 `heading`：140 個字元。
	Trigger string `json:"trigger"`
}
