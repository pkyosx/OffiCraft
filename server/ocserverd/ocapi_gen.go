...
// LoreEntryDetailDTO One lore entry in full — 六格 — plus the original preserved beside it.
type LoreEntryDetailDTO struct {
	// Content `content`（內容）— the compressed body, and the only cell that enters a boot context.
	...
	// Impact `impact`— 沒有這條記憶的人，**最糟**會發生什麼（owner 2026-09-06 逐字定義；⚠️ 它問的不是寫的人 —— 這件事有沒有真的發生完全不相干，被當場擋下來的一樣寫得出來。而且問的是**最糟**，不是平均、也不是這次剛好的結果）。v8 之前這一格叫 `problem`，問的是起因。May be empty; it is optional on write.
	Impact string `json:"impact"`

	// ImpactStars `impact_stars`：1 = 做白工，原本要完成的目的沒達到｜2 = 弄壞的只有你動的那個｜3 = 把其他東西弄壞了。三級不是累加：把自己動的那個修好了、卻炸了其他人，那是 3 不是 2；分界只有一條 —— 壞掉的東西在不在你動的範圍內。🔴 0 不是最輕的一級，是「還沒判」。從 2026-09-06 起新寫入不准送 0（owner 逐字：「不允許給 0」），所以**讀到 0 只有一種來源：v8 之前寫下的存量條目**。把 0 讀成 1 等於替它們做了一次沒有人做過的判定。
	ImpactStars int `json:"impact_stars"`
	...
	// Reviewed 審核旗標：這條的星等有沒有被人蓋過章。🔴 它跟 `impact_stars` 是兩欄而不是一欄，因為星等是 agent 自己提的，合成一欄就是讓 agent 自己蓋自己的章。⚠️ 這一版**沒有任何路由改得動它**，所以它現在對每一條條目都是 false —— 誰能蓋章、蓋章要不要留下是誰蓋的，都還沒有人裁定，而在裁定之前先開一道門，等於讓實作把那個決定偷偷做掉。
	Reviewed bool `json:"reviewed"`

	// Revisions The revision catalogue, oldest first, without any text.
	Revisions []LoreRevisionRowDTO `json:"revisions"`

	// RevisitWhen `revisit_when`（什麼情況出現時要把這一條拿出來**重新判一次**）— free text, may be empty. ⚠️ It does NOT ask when the entry becomes WRONG: the condition coming true means only that somebody must look at this entry again BEFORE trusting it next, not that it has already failed. Blank means nobody has said, not 「永遠都需要」 — and the cost of blank is that this entry keeps being handed to people with nobody knowing when to look at it again, including long after it stopped holding.
...
type LoreEntryRefDTO struct {
	...
	// Heading 標題格 — 「發生了什麼」, capped at 140 characters (Unicode runes). This row carries the 標題 because it is the cheapest thing that answers 「what is filed under this name」 — it is the cell designed to be read alone. ⚠️ 這一行以前帶的是 `trigger`，而列表畫面帶的是 `heading` ⇒ 審核者在待審佇列上看到的那一行，跟他在列表上看到的同一條記憶不是同一句話。owner ruling rc-9002654dd81c (2026-09-06), verbatim 「合併成 heading 一格（同時把搜尋改成掃 heading＋內容、待審畫面改顯示 heading）」。
	Heading string `json:"heading"`
...
// LoreEventDTO ONE event under a lore entry — `events`, 時／事／人／地／物. 🔴 THERE IS NO EVENT ID HERE, AND THAT IS NOT AN OVERSIGHT: nothing addresses a single event yet — no route edits one, no route deletes one — and an id nobody can use would be a handle that looks like a capability.
type LoreEventDTO struct {
	// Actor 人 — a `type:name` key (`human:Seth`, `agent:Kyle`). OPTIONAL: send it only when you know. A non-empty value must be `type:name` with a type prefix drawn from the SAME approved list `subjects` uses, refused BY NAME otherwise; an empty one is accepted as-is and never back-filled with a placeholder. Empty when nobody knew it.
	Actor string `json:"actor"`

	// HappenedTs REQUIRED. When the event HAPPENED, epoch seconds — not when this row was written. The two differ by days the moment somebody records last week's incident today, and a store that conflates them turns `events` into a record of the writing order rather than of what happened. Zero or negative is refused; there is no default, because 「now」 would be a claim nobody made.
...
// LoreEventInputDTO ONE event to file under a lore entry — `events`, 時／事／人／地／物. 時 and 事 are required; 人／地／物 are sent ONLY when you actually know them. Leaving one out is the correct way to say 「我不知道」 — do not write 「未知」 / `unknown` / `n/a` into it, because that makes 「查不出來」 and 「還沒有人去查」 permanently indistinguishable. The field set is CLOSED; an unknown key is a 422.
type LoreEventInputDTO struct {
	// Actor 人 — a `type:name` key (`human:Seth`, `agent:Kyle`). OPTIONAL: send it only when you know. A non-empty value must be `type:name` with a type prefix drawn from the SAME approved list `subjects` uses, refused BY NAME otherwise; an empty one is accepted as-is and never back-filled with a placeholder.
...
type LoreProposalAppliedDTO struct {
	...
	// EventsAfter How many events `events` carries NOW. The list was replaced wholesale by the proposal's own, so this is that proposal's count rather than a sum — a smaller number than before means the acceptance removed events, which is exactly what a merge could never do.
...
type LoreProposalDTO struct {
	...
	// Content The proposed `content`（內容）. Empty on a `remove`.
	Content string `json:"content"`
	...
	// Heading The proposed 標題格 — what a reader sees in a list and decides on. Empty on a `remove`.
	Heading string `json:"heading"`

	// Impact The proposed `impact`. Empty on a `remove`.
	Impact string `json:"impact"`

	// ImpactStars The proposed 星等 (0..3) for `impact` — the entry's importance. 0 on a `remove`, and 0 on an `update` means 還沒判.
	ImpactStars int `json:"impact_stars"`
	...
	// RevisitWhen The proposed `revisit_when`（什麼情況出現時要把這一條拿出來重新判一次）. Empty on a `remove`.
	RevisitWhen string `json:"revisit_when"`
...
type LoreProposalListDTO struct {
	// CurrentEvents The entry's `events` AS IT STANDS RIGHT NOW, in the order things happened — the other side of every proposal's `events_added` / `events_removed`. A reviewer holding both lists recomputes the difference instead of trusting it, which is the rule `current_sha256` already follows for `stale`.
...
type LoreProposeDTO struct {
	...
	// Content The proposed `content`（內容）— the one cell that ever enters a boot context. REQUIRED on an `update`, blank is refused, exactly as on a write. Sent only on an `update`, where the body cells together with `events` are the whole new version; on a `remove` it must be absent or empty.
	Content *string `json:"content,omitempty"`
...
type LoreSearchDTO struct {
	// Limit How many entries to return, 1..100, default 20. Out of range is REFUSED, never clamped: a caller that asked for 500 and silently got 100 believes it has seen everything.
	Limit *int `json:"limit,omitempty"`

	// Query A LITERAL, case-insensitive substring over 標題格 (`heading`) and 內容格 (`content`). Not semantic — `applied.query_match` reports which kind it was, so nothing has to guess. 🔴 掃描面以前是 `trigger`＋`content`，而列表與待審畫面顯示的是 `heading` ⇒ **使用者看到的那一行，搜尋搜不到**，而搜不到跟「站上真的沒有這條」長得一模一樣。owner ruling rc-9002654dd81c (2026-09-06), verbatim 「合併成 heading 一格（同時把搜尋改成掃 heading＋內容、待審畫面改顯示 heading）」 讓名字與軸變成同一格，那個落差在構造上消失了。🔴 第 3、4、5 格 (`revisit_when`, `impact`, events) are deliberately NOT searched: widening what `query` answers is a decision nobody has made, and the symptom would be extra hits that look exactly like correct ones.
...
type LoreSearchResultDTO struct {
	...
	// Entries The entries, most specific first (fewest subjects of their own), then oldest, then by id. ⚠️ `human:` origins used to sort ahead of everything within a tier; `origin` was removed on 2026-09-06 (rc-9c9bf14a579f) and that key went with it.
