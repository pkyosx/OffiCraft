...
type LoreEntryDetailDTO struct {
	...
	// RetireWhen `retire_when`（什麼時候不需要了）— free text, may be empty. Blank means nobody has said, not 「永遠都需要」.
...
type LoreProposalDTO struct {
	...
	// RetireWhen The proposed `retire_when`（什麼時候不需要了）. Empty on a `remove`.
	RetireWhen string `json:"retire_when"`
...
type LoreProposeDTO struct {
	...
	// RetireWhen The proposed `retire_when`（什麼時候不需要了）— free text. Optional, exactly as on a write. Sent only on an `update`, where the body cells together with `events` are the whole new version; on a `remove` it must be absent or empty.
