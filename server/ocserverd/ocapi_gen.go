...
// LoreEntryDetailDTO One lore entry in full — 標題格與內容格，加上 `events` — plus the original preserved beside it. ⚠️ 它以前是六格：`revisit_when` / `impact` / `impact_stars` / `supersedes` 四格由 owner 2026-09-06 逐字「都改掉」(訊息 c-3d3e5582c2d2) 連同資料庫欄位一起拿掉了。
type LoreEntryDetailDTO struct {
	...
	// Reviewed 審核旗標。🔴 它存在的唯一理由是替 `impact_stars` 蓋章，而那一格已經被 owner 2026-09-06 逐字「都改掉」(訊息 c-3d3e5582c2d2) 拿掉了 ⇒ **它今天沒有任何東西可以蓋章**。它留在原地是因為 owner 只點名了四格、沒有點名它，不是因為它還有用。⚠️ 這一版**沒有任何路由改得動它**，所以它現在對每一條條目都是 false。
...
type LoreSearchDTO struct {
	...
	// Query A LITERAL, case-insensitive substring over 標題格 (`heading`) and 內容格 (`content`). Not semantic — `applied.query_match` reports which kind it was, so nothing has to guess. 🔴 掃描面以前是 `trigger`＋`content`，而列表與待審畫面顯示的是 `heading` ⇒ **使用者看到的那一行，搜尋搜不到**，而搜不到跟「站上真的沒有這條」長得一模一樣。owner ruling rc-9002654dd81c (2026-09-06), verbatim 「合併成 heading 一格（同時把搜尋改成掃 heading＋內容、待審畫面改顯示 heading）」 讓名字與軸變成同一格，那個落差在構造上消失了。🔴 `events` is deliberately NOT searched: widening what `query` answers is a decision nobody has made, and the symptom would be extra hits that look exactly like correct ones.
...
// LoreWriteDTO Create one lore entry. The fields are named here rather than counted, because a count goes stale silently: `heading`, `content`, `subjects` and `events` are what this route takes today — the first three REQUIRED, `events` 0..N. ⚠️ IF THAT LIST IS WRONG, IT IS WRONG IN A WAY YOU CAN FIND: a field the route accepts but this sentence does not name is a name missing from a list, which greps. An earlier version of this sentence said 「五格 (a heading, three body cells plus 0..N events)」 and stayed wrong through two field removals without anything noticing, because a number points at nothing. The field set is CLOSED — an unknown key is a 422, never a silent drop.
