...
type AgentRelocateReceiptDTO struct {
	...
	// RelocationDeferred WHICH of ``relocation_pending``'s two causes this is: true means a deliberately deferred move — the agent's session is still live on the old machine, so the move waits for that session to end — rather than a delivery failure. The field does not distinguish the ways that happens, because the consumer's question is the same in all of them: (a) THIS relocate opened a graceful wind-down, and the move lands when the agent answers report_stopped; (b) an EXISTING wind-down at a HIGHER rung of the 停止 → 加速停止 → 強制停止 ladder already owns the agent, so the pin was saved and the ladder refused to re-open a lower stage — the move lands at THAT wind-down's collect, on whatever deadline it already carries (T-170e); (c) an outsource worker whose wind-down is already collected: its old session has been sent a STOP, and the START on the pin goes out once the worker reads offline. A caller must hold back the "nothing was dispatched" alert for every case; the cockpit does exactly that.
	RelocationDeferred *bool `json:"relocation_deferred,omitempty"`

	// RelocationPending True when the move is SCHEDULED BUT NOT LANDED. The pin itself is persisted before any dispatch, so a relocate never fails on dispatch — and that is what made a clean 200 dangerous. Two different non-landings reach this flag: a decided recycle STOP/START the target warden would not accept, and a move deferred by design (see ``relocation_deferred``). Absent means nothing was left undelivered; it does NOT mean the agent is already running on the pin. The cadence retries the pinned move regardless.
...
// InsightReplaceDTO Whole-doc replace of an insight doc: “{text}“. “text“ is REQUIRED — a whole-doc replace must never infer "empty" from a missing key. “allow_shrink“ (default false) must be set explicitly to replace existing content with an empty doc.
type InsightReplaceDTO struct {
	AllowShrink *bool  `json:"allow_shrink,omitempty"`
	Text        string `json:"text"`
}

// LessonsEditDTO One anchor-addressed edit (§3.4 #28b): replace the occurrence of “old“ with “new“. “old“ must match the current doc EXACTLY ONCE (0 or >1 hits reject the whole batch — the unique anchor doubles as an optimistic concurrency check); an EMPTY “old“ appends “new“ at the end of the doc (joined with a newline when the doc does not already end in one).
...
type LoreEntryDTO struct {
	...
	// ScopeKey The writer's own member id when ``scope_kind`` is ``agent``; the task manual's ``type_key`` when it is ``manual``; "" when it is ``everyone``. A surviving legacy ``role`` row (see ``scope_kind``) still carries a role_key here.
	ScopeKey string `json:"scope_key"`

	// ScopeKind ``agent``, ``manual`` or ``everyone``, and they are not interchangeable. An ``agent`` entry rides ONE member's own boot document — staff and outsource alike; a ``manual`` entry rides ``get_task_manual``; an ``everyone`` entry (所有人) rides the boot document of EVERY member — staff, outsource and mira alike.
	//
...
// LoreEntryScopeDTO The set_lore_entry_scope request body (T-236): the TARGET scope kind and nothing else. The scope_key is derived by the server — the author's member id for “agent“, the entry's derivable task type_key for “manual“, "" for “everyone“ — so a caller cannot point an entry at another member's boot document or at an unrelated manual.
type LoreEntryScopeDTO struct {
	// ScopeKind The TARGET scope, one of ``agent``, ``manual``, ``everyone``. Anything else — the retired ``role`` included — is a 400 that names it. ``manual`` is also a 400 when the entry has no derivable task type, and ``agent`` when its author has no roster row; either is then absent from the entry's ``scope_options``. There is no ``scope_key`` field: the server derives the key itself (the author's member id / the task type_key / "").
	ScopeKind string `json:"scope_kind"`
}

// LoreEntryScopeReceiptDTO Bounded receipt for “POST /api/lore/{entry_id}/scope“ (set_lore_entry_scope) (T-236), under the same owner ruling as “LoreEntryStateReceiptDTO“ (2026-09-07: 「不要回傳自己寫出去的 payload」): only the id and what the write itself decided ride home. “scope_key“ is the one value the caller could not send or predict — the server derived it. “state“, “effective_ts“ and “updated_ts“ carry the same meaning they do on the state/bump receipt, so the three governance doors answer with one vocabulary; this call never changes “state“ or “effective_ts“. “title“, “body“, “author_id“, “source_task_id“ and the timestamps of creation are dropped; call “list_lore_entries“ (“GET /api/lore“) for the entry itself.
type LoreEntryScopeReceiptDTO struct {
	// EffectiveTs The entry's ordering key, epoch seconds — untouched by a scope move.
	EffectiveTs float64 `json:"effective_ts"`

	// Id The entry that was moved, echoed from the path.
	Id string `json:"id"`

	// ScopeKey The key the SERVER derived: the author's member id for ``agent``, the task type_key for ``manual``, "" for ``everyone``.
	ScopeKey string `json:"scope_key"`

	// ScopeKind ``agent`` | ``manual`` | ``everyone`` — the scope the entry is in AFTER this write, read back from the stored row. Equal to the request when the call was a no-op.
	ScopeKind string `json:"scope_kind"`

	// State ``active`` | ``pinned`` | ``retired`` — unchanged by this call, read back from the stored row.
	State string `json:"state"`

	// UpdatedTs The stored row's last-write stamp, epoch seconds, as it stands after this call.
	UpdatedTs float64 `json:"updated_ts"`
}
...
type LoreEntryWriteReceiptDTO struct {
	...
	// Id ``L-<n>``, MINTED HERE, ``n`` ascending globally. The handle ``set_lore_entry_state``, ``bump_lore_entry`` and ``set_lore_entry_scope`` take as ``entry_id``, and the one thing the caller cannot compute.
	Id string `json:"id"`
...
type MemberDTO struct {
	...
	// RefocusOp Which owner operation opened the in-flight handover stamped in ``refocus_since``, empty when none is in flight. One of ``relocate`` (machine change), ``runtime/model`` (runtime / model / effort change), ``context_notice`` (FIRST context-pressure threshold), ``context_high`` (SECOND context-pressure threshold), ``refocus`` (owner-pressed refocus), ``restart_self`` (agent-requested), ``token_expiry`` (the session's agent token is inside its last hour — the close-out is opened while the calls that file it still work), ``accelerated_stop`` (the owner pressed 加速停止 on a wind-down that was already open) or ``task_close`` (the task this OUTSOURCE worker was bound to landed terminal, so its close-out window was opened). Stamped and cleared in lockstep with ``refocus_since``. THE CAUSE ALSO SAYS WHETHER ANYTHING IS ON A CLOCK: ``context_high`` and ``accelerated_stop`` are force-collected on a deadline and share one grace (``stop.accelerated_grace_secs``), and ``task_close`` is force-collected on its own (``task.close_winddown_secs``, a separate value because nobody pressed a button for it); every other cause is collected by the agent's own ``report_stopped`` or by the owner pressing force-stop and carries none (owner 2026-08-21). A row can be promoted ``context_notice`` → ``context_high`` in place when context keeps climbing, which restamps ``refocus_since``. WAS: the cause lived only in a server log line, so a client could only say 'last refocus' — which reads as history — where it meant 'winding down right now so your change can take effect' (T-7f28). Additive-optional.
...
type ResumeTaskDTO struct {
	...
	// ReassignedFromKind The kind of ``reassigned_from`` (``staff`` | ``outsource``), so the id is resolved the right way (roster row vs outsource worker). ``''`` whenever ``reassigned_from`` is ``''``.
...
type SettingsDTO struct {
	...
	// DocCapCharsManualSop The size cap on a TASK MANUAL's SOP doc (sop_md), in CHARACTERS (Unicode code points). The floor of the adjustable range is this segment's shipped default (the `default` field above), the ceiling is 100000.
	...
	// TaskCloseWinddownSecs Task-close wind-down, in seconds (10 through 3600): how long an OUTSOURCE worker is left running after the task it is bound to lands terminal — all four closes alike (mark_task_done, mark_task_terminated, mark_task_duplicated, force_task_done) — before the server takes its session and releases its row. Through that window the worker reads 停止中 in the cockpit and is STILL ON THE PANEL; it leaves the panel only when it is actually collected. The worker can ask to be collected early by reporting stopped, but that report is a REQUEST and not a proof: the collection still waits for the server's own offline determination to say the session is gone (owner 2026-09-20, rc-604d8fc39cfd). A worker that is already offline when the task closes never enters the window — it is collected on the spot. Same bounds as accelerated_grace_secs, a SEPARATE value: this one sizes a close-out nobody pressed a button for, and moving one must not move the other.
	TaskCloseWinddownSecs *int  `json:"task_close_winddown_secs,omitempty"`
...
type SettingsUpdateDTO struct {
	// AcceleratedGraceSecs 加速停止 grace, in seconds. Must be 10 through 3600. Applies to both of those causes at once (the second context threshold and the owner-pressed 加速停止); it can never put a clock on a soft cause, and it is not the outsource task-close window, which carries its own value (task.close_winddown_secs).
	AcceleratedGraceSecs *int `json:"accelerated_grace_secs,omitempty"`
	...
	// TaskCloseWinddownSecs Task-close wind-down, in seconds. Must be 10 through 3600. How long an OUTSOURCE worker keeps running after its task is closed before the server collects it; it reads 停止中 for that whole window and leaves the panel only once collected. A stopped-report shortens the window but does not by itself end it — the server's own offline determination does.
	TaskCloseWinddownSecs *int  `json:"task_close_winddown_secs,omitempty"`
	UpdaterAutoUpdate     *bool `json:"updater_auto_update,omitempty"`
	UpdaterReceiveBeta    *bool `json:"updater_receive_beta,omitempty"`
...
type TaskDTO struct {
	...
	// ReassignedFromKind The kind of ``reassigned_from`` ("staff" | "outsource"), so the client resolves the id the right way (roster name vs outsource codename). "" whenever reassigned_from is "". additive-optional (T-ba04).
	ReassignedFromKind *string       `json:"reassigned_from_kind,omitempty"`
	Status             string        `json:"status"`
...
type TaskForceDoneDTO struct {
	// Reason Why this task is being closed without its steps saying so. OPTIONAL: omit it, or send a blank/whitespace-only string, and the task still closes with ``forced_done_reason`` ''. When given it is trimmed and stored.
	Reason *string `json:"reason,omitempty"`
}
...
type TaskListItemDTO struct {
	...
	// ReassignedFromKind The kind of ``reassigned_from`` ("staff" | "outsource"). "" whenever reassigned_from is "". additive-optional (T-ba04).
	ReassignedFromKind *string  `json:"reassigned_from_kind,omitempty"`
	Status             string   `json:"status"`
	TaskNo             string   `json:"task_no"`
	Title              *string  `json:"title,omitempty"`
...
type TaskManualDTO struct {
	...
	// LoreChars Size of ``lore`` in CHARACTERS — the block the server assembled for this read, which is a different thing from ``sop_md_chars`` (what a writer may edit).
	LoreChars *int    `json:"lore_chars,omitempty"`
	Purpose   *string `json:"purpose,omitempty"`
	SopMd     *string `json:"sop_md,omitempty"`

	// SopMdCapChars The cap on `sop_md` now in force, in CHARACTERS (the doc.cap_chars.manual_sop setting).
	SopMdCapChars *int `json:"sop_md_cap_chars,omitempty"`

	// SopMdChars Size of `sop_md` in CHARACTERS.
...
// TaskManualDocSizesDTO The capped document of ONE task manual, by size only: “sop“ (sop_md).
...
type TaskManualReceiptDTO struct {
	...
	// SopMdChars Size of the SOP document as stored, in CHARACTERS. Present ONLY when this call wrote the SOP document; absent otherwise. No ``default``, not required.
	SopMdChars *int `json:"sop_md_chars,omitempty"`
...
// TaskManualUpdateDTO Partial manual edit — only supplied fields change. “assignee“ is {"kind":"staff","member_id":…} or {"kind":"outsource","runtime":"claude|codex","model":…,"effort":…,"copies":N,"machine":…}; {} unsets it. Outsource runtime absent means claude; “copies“ MUST be a number >= 0 (0 = 無限 — unlimited per-type parallel copies; absent = 1); “machine“ MUST be a non-blank machine id when present, and must resolve to a real machine ("auto" is rejected — it names no machine); absent leaves the type without a placement.
...
type TaskStepDTO struct {
	...
	// ReplyCardStatus Read-time join: the CURRENT status (``waiting`` | ``answered`` | ``expired``) of the reply card bound to this step (``reply_card_id``); ``""`` when the step carries no card. Lets the task-embedded card (TaskReplyCard) decide AT MOUNT whether to load eagerly (waiting — the live ask / the H4 answered-awaiting-pickup transitional) or lazily (answered — collapsed one-line summary, fetch on expand) WITHOUT a per-card GET, and lets the board derive the H4 badge without the child round-trip. NOT stored — computed each read from the card's live status.
	ReplyCardStatus *string  `json:"reply_card_status,omitempty"`
...
type TaskStepDetailDTO struct {
	...
	// ReplyCardStatus Read-time join: the CURRENT status (``waiting`` | ``answered`` | ``expired``) of the reply card bound to this step; ``""`` when the step carries no card. Same computation as TaskStepDTO.reply_card_status.
	ReplyCardStatus *string  `json:"reply_card_status,omitempty"`
	StartedTs       *float64 `json:"started_ts,omitempty"`
	Status          string   `json:"status"`
	TaskId          string   `json:"task_id"`
...
type TaskStepNoteReceiptDTO struct {
	...
	// Sha256 Hex sha256 over the note AS STORED after this write (T-91). Present so a caller can confirm what landed WITHOUT the text riding back - the same verification anchor patch_step_note, patch_insight and patch_task_sop already carry.
	Sha256 *string `json:"sha256,omitempty"`
...
type TaskWriteReceiptDTO struct {
	...
	// Lock The handover lock, ``reassigning`` while a transfer is waiting to be claimed and empty otherwise. Only reassign_task sets it and only claim_task clears it, so on the other four verbs it is the answer to a question with no other cheap source: ``status`` is derived from the steps and does not move when a lock is placed, so a caller reading status alone cannot see that the ticket is mid-transfer.
	Lock *string `json:"lock,omitempty"`
	...
	// Title The ticket's title AFTER this write. It is NEWS on five of the six verbs and an echo on one, and the split is worth stating because owner asked exactly this question about create_task on rc-bf25374aa0e8. claim_task, reassign_task, mark_task_done, mark_task_terminated, mark_task_duplicated, force_task_done and set_task_deps are all called with a task id and no title, so the caller may never have seen the ticket it just acted on - the title is how a person recognises which one. update_task (and the title twin) is the exception: there the caller sent it, and the server TRIMS what it sent, so even there the value can differ from what was posted. ``waiting_reason`` was on an earlier draft and has been REMOVED: reassign_task and mark_task_duplicated stamp it empty unconditionally, the other four never touch it so it is a stale read, and no caller anywhere reads it off a write.
	Title *string `json:"title,omitempty"`
}
...
type HandleListLoreEntriesApiLoreGetParams struct {
	...
	// ScopeKind Single-value twin of ``scope_kinds``. Accepted values: ``agent``, ``manual``, ``everyone``; any other value — the retired ``role`` included — is a 400 that names it. Ignored when ``scope_kinds`` is sent (PLURAL WINS).
...
// HandleSetLoreEntryScopeApiLoreEntryIdScopePostJSONRequestBody defines body for HandleSetLoreEntryScopeApiLoreEntryIdScopePost for application/json ContentType.
type HandleSetLoreEntryScopeApiLoreEntryIdScopePostJSONRequestBody = LoreEntryScopeDTO
...
type ServerInterface interface {
	...
	// Replace the EDITABLE HALF of the 啟動步驟 block of ONE runtime ({runtime_key, body}). runtime_key is 'claude' or 'codex' and the two are separate documents that are not interchangeable (only the codex one ends its boot turn by handing control back to the sidecar that holds its connection), so writing the wrong one leaves those agents with a boot sequence written for the other runtime — and nothing that never boots reports it. body is REQUIRED and unknown keys are rejected; emptying a body that had content needs allow_shrink=true. Neither runtime's document carries a read-only head today (T-6f44, owner's decision 4 removed it), so the body IS the whole document; the head machinery still exists for the kinds that do carry one, and there is no way to write a head on any face. The stored result is judged against the doc.cap_chars.boot_sequence cap (one cap, both runtimes, each measured on its own text); the refusal tells you what you wrote, the cap, and what is stored. The shipped seed is never overwritten, so reset_boot_sequence always gets the factory text back. Owner or admin assistant only. Answers with a bounded receipt (“kind“, “key“, “is_default“, “size_chars“, “cap_chars“, “sha256“), not the document — call “get_boot_sequence“ when you need the rest.
	...
	// List 傳承 entries, filtered SERVER-SIDE and paged in the fixed order pinned -> active -> retired, newest first inside each group. The order is not configurable; the filter is. Nothing is narrowed by who is calling: with no author or scope filter, every caller sees every entry.
	//
	// PARAMETER NOTES. In the input schema the parameters below carry only a short summary; these are their full rules.
	...
	// (POST /api/lore/{entry_id}/scope)
	HandleSetLoreEntryScopeApiLoreEntryIdScopePost(w http.ResponseWriter, r *http.Request, entryId string)
	// Move one 傳承 entry to active / pinned / retired. 置頂 and un-置頂 are ADMIN-ONLY (owner ruling): a pinned entry sorts ahead of every other entry in its scope and so survives the cap at the others' expense. 失效 and 生效 are open to the entry's own AUTHOR -- anyone else is a 403 -- and admin capability is unrestricted. A PINNED entry is outside its author's reach as well: until an admin un-pins it, every state change a non-admin requests on it, the author's included, is a 403. The author can still move it to the top of its group with “bump_lore_entry“. Retiring is not deleting: the entry keeps its id and can be moved back; “retire_reason“ is stored only with retired and cleared by the other two.
	...
	// List every member that has not been removed, including outsource members and warden rows (kind=warden, one per machine) by default (presence-derived MemberDTO[]). A warden row is not a chat address: post_chat to its id is a 404. fields=light returns an identity-only projection that preserves kind.
	// (GET /api/members)
	...
	// Read one reply card (full context: options, status, answer). status is waiting, answered or expired; an expired card was closed without an answer. Any authenticated caller may read any card by id, not only a card it opened or one bound to its own task; an unknown id is a 404.
	...
	// - `accelerated_grace_secs`: 加速停止 grace, in seconds. Must be 10 through 3600. Applies to both of those causes at once (the second context threshold and the owner-pressed 加速停止); it can never put a clock on a soft cause, and it is not the outsource task-close window, which carries its own value (task.close_winddown_secs).
	...
	// Report a step status (pending/in_progress/waiting_external/done). Reporting waiting_owner or superseded is a 400 (neither is agent-reportable), a transition the step state machine does not allow is a 409, and entering waiting_external requires a non-blank waiting_reason (422 otherwise); the task status is derived from its steps. A report that leaves every step that is not superseded done lands the task in “ready_for_done“, which does not close it: a task closes only through “mark_task_done“, “force_task_done“, “mark_task_terminated“ or “mark_task_duplicated“, and once the task is closed it can never be replanned (submit_plan becomes a permanent 409).
...
// HandleSetLoreEntryScopeApiLoreEntryIdScopePost operation middleware
func (siw *ServerInterfaceWrapper) HandleSetLoreEntryScopeApiLoreEntryIdScopePost(w http.ResponseWriter, r *http.Request) {
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.HandleSetLoreEntryScopeApiLoreEntryIdScopePost(w, r, entryId)
	}))
...
func HandlerWithOptions(si ServerInterface, options StdHTTPServerOptions) http.Handler {
	...
	m.HandleFunc(http.MethodPost+" "+options.BaseURL+"/api/lore/{entry_id}/scope", wrapper.HandleSetLoreEntryScopeApiLoreEntryIdScopePost)
