...
type LoreEntryDetailDTO struct {
	...
	// Impact 第 4 格「impact」— 原本想達成什麼、實際變成什麼。v8 之前這一格叫 `problem`，問的是起因。May be empty; it is optional on write.
	Impact string `json:"impact"`

	// ImpactStars 第 4 格的星等：1 = 沒弄壞任何東西｜2 = 弄壞的只有你動的那個｜3 = 弄壞的包含你沒動的。🔴 0 不是最輕的一級，是「還沒判」—— v8 之前的條目、以及沒有填星等的寫入，都是 0；把 0 讀成 1 等於替它們做了一次沒有人做過的判定。
...
type LoreProposalDTO struct {
	...
	// Impact The proposed 第 4 格「impact」. Empty on a `remove`.
	Impact string `json:"impact"`

	// ImpactStars The proposed 星等 (0..3) for 第 4 格 — the entry's importance. 0 on a `remove`, and 0 on an `update` means 還沒判.
	ImpactStars int `json:"impact_stars"`
...
type LoreProposeDTO struct {
	...
	// Content The proposed 第 2 格「內容」— the one cell that ever enters a boot context. REQUIRED on an `update`, blank is refused, exactly as on a write. Sent only on an `update`, where the body cells together with `events` are the whole new version; on a `remove` it must be absent or empty.
	Content *string `json:"content,omitempty"`
	...
	// RetireWhen The proposed 第 3 格「什麼時候不需要了」— free text. Optional, exactly as on a write. Sent only on an `update`, where the body cells together with `events` are the whole new version; on a `remove` it must be absent or empty.
...
type LoreSearchHitDTO struct {
	...
	// ImpactStars 第 4 格的星等 0..3 — **the entry's importance** (owner 2026-09-05: 「評分也改了不用 用星等取代 因為 impact 本就是重要性」). It travels on this row because a list of titles with no importance can only be read in order, and order is not importance. 0 means 還沒判, not 「lightest」.
	ImpactStars int `json:"impact_stars"`
