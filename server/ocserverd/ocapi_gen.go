...
// TaskStepStatusUpdateDTO Agent-reported step status (MCP “update_step_status“): “pending“ → “in_progress“ → “done“ — “waiting_owner“ is NOT agent-reportable on either side (a step enters it only by opening a reply card: create_reply_card carrying an explicit “linked_task“ {task_id, step_id}, and leaves it only when that card is answered, where the server restores in_progress), so reporting “waiting_owner“ is a 400 and a move out of it is a 409; other illegal transitions are a 409. “superseded“ is likewise not the agent's lever: the server freezes a replaced answered-card step itself on submit_plan (T-1aea), so reporting “superseded“ is a 400 and no report moves a step out of it (409 — terminal). T-74f8 交棒閘: if applying this report would FINISH the task (every step that is not superseded done, so the task lands in “ready_for_done“; reaching “ready_for_done“ never closes the task, which closes only through “mark_task_done“, “force_task_done“, “mark_task_terminated“ or “mark_task_duplicated“) and the task's creator is not its executor, the server refuses the report with a 422 unless the ball's destination is declared IN THIS SAME CALL — “handoff='return_to_creator'“ (recorded on this task and nothing else: no task is opened and nobody is notified; a 422 when the creator is not an active roster member, which includes every task the owner created), “handoff='follow_up'“ + “handoff_task_id“ (the server attaches this task to that successor as a dependency), or “handoff='none'“ + “handoff_note“ (an explicit, recorded end of the line). The gate stands aside when a non-terminal task already depends on this one (the handover is already real). It refuses BEFORE any row is written. This report is the only call that carries a declaration: neither submit_plan nor mark_task_done accepts one, and a closed task can never be replanned (submit_plan turns into a permanent 409).
type TaskStepStatusUpdateDTO struct {
	// Handoff Where the ball goes when this report finishes the task (every step that is not superseded done): ``return_to_creator`` | ``follow_up`` | ``none``. Read only on that report.
	Handoff *string `json:"handoff,omitempty"`
...
type ServerInterface interface {
	...
	// Post a chat message (sender = verified JWT sub; auto SSE fan-out). “to“ must name the owner or an active staff or outsource member; an unknown id, a removed member, a warden (machine) id and a server-generated sender address (“hook:<endpoint_id>“, “sched:<schedule_id>“ or “system“) are all a 404. Presence is not a gate: an offline member keeps its durable mailbox. Answers with a bounded receipt (“id“, “ts“, “to“, “attachments“), not the message — call “get_chat“ when you need the rest.
	// (POST /api/chat)
	...
	// List 傳承 entries, filtered SERVER-SIDE and paged in the fixed order pinned -> active -> retired, newest first inside each group. The order is not configurable; the filter is. Nothing is narrowed by who is calling: with no author or scope filter, every caller sees the entries of every member and every manual.
	// (GET /api/lore)
