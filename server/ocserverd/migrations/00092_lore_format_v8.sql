...
-- 狀態的寫法。
--
-- ── 🔴 正式庫 goose ledger 的當下量測，以及它回答不了的那一題 ─────────────────
--
-- 這一支底下有好幾段話依賴「某一支 migration 在正式站跑過沒有」。那一題只有正式庫
-- 的 `goose_db_version` 表答得出來，而**這個檔以前是從自己的檔名推的** —— 那些句子
-- 一路帶著舊號碼（00081／00082／00083）與一句「goose 停在 83」，全部是假的。
--
-- 唯一一次真的去讀那顆資料庫的量測（2026-09-07，量測者是**銀月**，不是這個檔的作者
-- 也不是 Kyle；`/Users/seth_wang/.officraft/server/data/officraft.db`，唯讀開啟、
-- 沒有啟動測試站、沒有寫入）：
--
--   `goose_db_version` 最新一列 version_id = 86（tstamp 2026-09-06 02:58:22）
--   80–92 這個區間裡只有 **80** 與 **86** 兩列 is_applied=1
--   81 / 82 / 83 / 84 / 85 / 87 / 88 / 89 / 90 / 91 / 92 —— 一列都沒有
--
-- 🔴 **這是一次量測，不是一個保證，而且那個 86 明天就可能不是 86。** 要用的是查法
-- 不是數字：去讀正式庫的 `goose_db_version`，不要沿用上面這幾個數。
--
-- 🔴🔴 而它回答不了的那一題是：**「沒有列」跟「從來沒有套用過」在這張表上長得一模
-- 一樣。** owner 在 2026-09-06 **刪掉**了其中幾列，所以一支 migration 可能已經動過
-- 那顆資料庫、只是紀錄被移走了。⇒ 底下任何一段都不可以從「ledger 裡沒有它」跳到
-- 「所以它沒跑過、所以就地改是安全的」。這個檔以前正是這樣跳的。
--
-- ── 🔴 舊號對照：底下有幾段話裡的號碼是**改號之前**的，而它們是對的 ─────────────
--
-- 這條分支的四支 migration 原本是 00081–00084，後來整批改成 **00089–00092**
-- （00081→00089、00082→00090、00083→00091、00084→00092）；T-79 當時拿到的 00085
-- 現在是 00087。
-- ⚠️ 底下「為什麼不另開 00085」與它下面那兩次掃機器的量測（25 顆 DB、31 顆 DB）
-- **整段沿用舊號，而且不應該被改成新號** —— 那些數字是那幾顆資料庫**當時真的記著
-- 的值**，改成新號會把一份量測紀錄改成一句沒有人量過的話。讀那幾段的時候把上面這
-- 張對照表放在旁邊。
--
...
--
-- `problem` 是**本樹的 00089** 建的（`CREATE TABLE lore_entry` 在那一支，已核對）。
-- 一支已經套用過的 migration 不能就地改（它的 sha256 坐在 migration.lock 的中段）
-- ⇒ 這一格只能由這一支 ALTER 掉。
-- 🔴 這個結論**不是**由「ledger 說 00089 套用過」推出來的 —— 檔頭那一段已經講了，
-- 這張表分不出「沒跑過」與「跑過但紀錄被刪」。這裡採 ALTER 的理由是**代價不對稱**：
-- 就地改而它其實跑過，升級站與全新站會從此帶著不同 schema、兩邊都認為自己是對的，
-- 而且不會報錯；走 ALTER 而它其實沒跑過，代價只是多一支不必要的 migration。
...
--
-- 🔴 Kyle（`c-5f576ae65f3d`）裁定並指名這個陷阱：goose **不會重跑一支已經套用過的
-- migration** ⇒ 把欄位補進 00089／00090／00091 這幾支之中的任何一支，對一顆已經跑
-- 過它們的資料庫等於什麼都沒發生，而且**沒有任何訊號**：本機的拋棄式 DB 會正確、
-- 正式庫會缺欄位、測試全綠。
-- ⚠️ 他當時寫的理由是「81/82/83 在正式庫 is_applied=1」（那是這三支的舊號）。
-- **那個前提今天不成立** —— 2026-09-07 銀月唯讀量測，那三個號碼在 ledger 裡一列都
-- 沒有（見檔頭）。但**裁定本身不受影響**：它靠的是「goose 不重跑」這條機制，而不是
-- 靠那三支的套用狀態；而且那個狀態現在無法判定（沒有列 ≠ 沒跑過）。
-- ⇒ 新欄位一律走這一支的 ALTER TABLE，即使那看起來比較醜。
--
-- 🔴 這一節到「兩顆 DB 的 goose MAX 是 85」為止，**號碼全部是改號前的**（對照表在檔頭）。
-- ── 為什麼不另開 00085 ──────────────────────────────────────────────────────
...
--
-- `origin` 是**本樹的 00089** 建的（`CREATE TABLE lore_entry` 在那一支，已核對）。
-- 走 ALTER 而不就地改 00089 的理由與上面 `problem` 那一段完全相同（代價不對稱，
-- 不是「ledger 說它套用過」）。
...
-- 一個值都不會掉；而這一格是**本樹的 00089** 建的，就地改它的代價不對稱 ⇒ 只能 ALTER
-- （理由同上面 `problem` 那一段）。
--
