...
- **篩選列照抄任務頁**（owner 2026-09-07：「我從使用者或是任務手冊作為 filter 另外就是狀態 三個而已」「你完全可以抄 task」「差別只是有多選沒有統計數字而已」）。**以設計稿為準**，設計稿上是：**傳承編號（搜尋框）｜ 撰寫人 ｜ 任務 ｜ 狀態 ｜ 清除篩選**。
  - 🔴 **第二格只放任務手冊，不放成員。** 成員那一軸是左邊的撰寫人；同一列放兩個問同一件事的控制項，兩邊的答案可以互相矛盾。owner 2026-09-07 看到第二格裡列著成員時當場退回：「成員的 filter 不是左邊那個嗎 你第二個 filter 應該只需要放任務」。
