...
| `scope_kind` | TEXT | `role`、`agent` 或 `manual` |
| `scope_key` | TEXT | `role` ⇒ role_key；`agent` ⇒ 該成員自己的 member id；`manual` ⇒ task manual 的 `type_key` |
...
**角色傳承**（`scope_kind='role'`，`scope_key` ＝角色）
接在正職開機檔的「長期筆記」區塊後面。**只有正職**，外包沒有角色，那條路徑上 `LoreScopeRole` 指不到任何東西。

**成員傳承**（`scope_kind='agent'`，`scope_key` ＝該成員自己的 member id）
接在外包開機檔裡，位置與角色傳承對稱。owner 核可於卡 `rc-3c24fdc61ed3`：外包學到的東西原本沒有地方放，於是它每醒來一次就從頭開始一次。
它**共用角色傳承那個字數旋鈕**，不另開第四個 —— 兩者是同一種支出（一位讀者自己開機檔裡的傳承區塊，每次開機都付），兩個數字只會變成兩個地方做同一個決定。
...
- **寫一筆**：`title`、`body`、可空的 `task_id`。
  - `task_id` 空 ⇒ 寫進**呼叫者自己的開機檔**，寫到哪一個由身分決定：**有角色（正職）⇒ `scope_kind='role'`、`scope_key` ＝ role_key；沒有角色但有名冊身分（外包）⇒ `scope_kind='agent'`、`scope_key` ＝呼叫者自己的 member id**。
  ...
  - 🔴 **`type_key` 是空的（臨時任務）⇒ 跟沒填任務一樣，寫進呼叫者自己的開機檔**（owner 於 `rc-35db15f76c66` 更正：「臨時任務跟無關乎任何任務一樣都是給 NULL」）。
    這一格曾經是 400，理由是「默默歸到角色傳承會讓每個人開機都付字數、而需要它的類型永遠拿不到，且不報錯」。**那個危害是靠不報錯成立的**，所以現在的做法不是把它放行，是把「默默」拿掉：回應會帶一句 `scope_note`，逐字說出這一筆寫進了哪裡、以及為什麼不是任何一本手冊。**沒有那一句，這一格就要退回 400。**
...
- 範圍要能明確選到 `無 · 角色傳承`；**條目列上要標「屬於」，而且標在收合的列上**（owner 卡 `rc-11734523eb52` 推翻了原本「不標」的規定：頁面預設打開是「全部」，各種範圍混在一起，讀的人分不出哪一筆是角色的、哪一筆是某本手冊的。標的格式是「是哪一種 · 是哪一個」——只標名字的話，一個叫「特助」的角色跟一本叫「特助」的手冊在列上長得一樣）
