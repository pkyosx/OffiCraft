...
11. **commit 訊息格式 `<type>(<scope>): <subject>` + `[why]` / `[how]` body**(owner 2026-07-28 `rc-a202dc39521c` 拍板,**全工作室統一格式,覆蓋本 repo 原本「`[why]` 開頭第一行」的舊寫法**;⚠️ 舊格式的 commit 在 `main` 上大量存在,那是歷史,不要回頭改寫):

    ```
    <type>(<scope>): <小寫祈使句 subject>

    [why]
    <1-3 句:為什麼要改,講清問題 / 動機,讓後人不必考古就懂意圖>

    [how]
    <2-5 句或條列:改了哪些關鍵處>
    ```

    - `type` 白名單:`feat` `fix` `docs` `refactor` `test` `chore` `ci` `style` `perf` `build`;`scope` 用域名(如 `frontend`、`ocserverd`、`spec`)。
    - subject **全英文、小寫開頭、祈使句現在式、句尾不加句點、≤ 72 字元**;**不放票號**(owner 2026-07-28 `rc-22758d4c734a`:標題就到描述為止)。
    - body **避免 emoji**;結尾固定一行 `Co-Authored-By: <實際執行的 Claude 模型名> <noreply@anthropic.com>`(例:`Claude Fable 5`、`Claude Opus 4.8`——署真實模型,不為符合慣例署不實名稱)。這條**不受新裁定影響,照舊保留**。
    - 理由:subject 一行講清「改了什麼類型的什麼」,`[why]` 讓未來的人與 AI 直接讀到改動意圖,對齊 §8 context-with-code。
