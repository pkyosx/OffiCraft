// CT story for the 「已用 / 上限」 readouts on the two task-manual documents
// (T-100), at phone and desktop widths.
//
...
} from "../../src/components/TaskManualsPage";
import type { TaskManualView } from "../../src/api/adapter";

/** Realistic content: a long CJK SOP and a long CJK learnings doc, with sizes
 * and caps in the range a real station actually carries (measured 2026-09-06:
 * 15796 / 18000 and 16999 / 17000). The five-digit pairs matter — a guard built
 * on 「1 / 10」 would never reach the width where the row runs out of room. */
const SOP = "先確認來源，再決定要不要動手。".repeat(40);
const LEARNINGS = "讀起來理所當然的通常沒價值；有價值的會糾正你的直覺。".repeat(40);
...
const MANUAL: TaskManualView = {
  ...
  sopMdChars: 15796,
  sopMdCapChars: 18000,
  learningsChars: 16999,
  learningsCapChars: 17000,
};

export function ManualDocUsageStory({
  doc,
  widthPx,
}: {
  doc: "definition" | "learnings";
  widthPx: number;
}) {
  return (
    <I18nProvider>
