...
// + owner CRUD. Reconcile-by-refetch on the "task_manual" SSE topic; every
// owner mutation ALSO refetches directly so the mock behaves identically.
//
...
// back so the 任務定義 sub-page could adopt it; the update receipt
...
// T-1170: `GET /api/task-manuals` answers a DIRECTORY — `sop_md` is not on it,
// only its size. So `manuals` is `TaskManualSummaryView[]`, a type that cannot
// carry the document, and the sub-page that renders it reads its manual through
// `useTaskManual` below. Everything else (the list page, the hub's 用途/欄位/負責成員 cards, the
...
interface UseTaskManuals {
  ...
  /** Partial update. RE-READS the list; the sub-page that renders the SOP
   * re-reads its own manual (useTaskManual.refetch). Returns
 ...
 * 任務定義 sub-page reads since T-1170 took `sop_md` off the list answer.
 *
 * Reconciles on the same "task_manual" topic the list listens on, so a
 * 版本紀錄 restore and another tab's edit both land here.
 *
