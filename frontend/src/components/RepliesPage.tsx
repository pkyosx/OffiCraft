...
export function RepliesPage({ replyCardId }: { replyCardId?: string }) {
   ...
   *
   * 候選 E-A 「三行 · 文字階層」. Owner 2026-09-10:「不對 你不用硬擠在一行」—
   * the previous cut bought compactness with 11.5px type, a 「·」-joined run and
   * a truncated task chip, and the four facts stopped being readable at a
   * glance. So the row spends VERTICAL space instead: three plain text lines,
   * no chips, nothing abbreviated.
   *
   *   1  the ask's title, 14px/600, WRAPPING rather than clipped
   *   2  the asker — a small avatar, the name, then the role, each its own
   *      element with real space between them and no 「·」 joining them
   *   3  the task, its FULL title, marked by a rule down its left edge
   *
   ...
   * be two empty cells; each is simply omitted, and with nothing joining the
   * cells there is no orphaned separator to clean up either.
   *
   ...
   * What is left is the thin rule the card hangs under. */
          ...
          <span className="reply-card__collapsed-main">
            ...
            <span className="reply-card__collapsed-meta">
              {/* A GLYPH, NOT ReplyCardAvatarButton: this row IS a button, and a
                  button inside a button is invalid HTML that browsers reflow
                  out of the row. The avatar-as-second-target (T-a706) still
                  lives on the expanded card's header, one click away. */}
              <span className="reply-card__collapsed-avatar" aria-hidden="true">
                <Avatar
                  size={20}
                  kind={avatarKindForMember(asker ?? { id: row.from })}
                  src={
                    (asker?.kind === "outsource"
                      ? undefined
                      : asker?.avatarUrl) ?? workerAvatarUrls.get(row.from)
                  }
                />
              </span>
              <span
