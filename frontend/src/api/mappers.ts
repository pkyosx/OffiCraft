...
export function toServerSettings(w: WireServerSettings): ServerSettingsView {
  return {
    ...
    // 7776000 (90 days) is the server's shipped default, the value a fleet that
    ...
    // old to send the field reads here exactly as the machines behave. It was
    // 2592000 (30 days) until T-fc53 第二段, which moved the default and made the
    // same number the credential's expiry: a stale copy here would now show the
    // owner a lifetime no machine on that station keeps.
    wardenCredentialLifetimeSecs: w.warden_credential_lifetime_secs ?? 7776000,
