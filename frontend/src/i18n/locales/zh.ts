...
export const zh = {
  ...
  tasks: {
    ...
    filterIdNoun: "編號",
    // ── 按編號找的三種結局(T-93 第三輪, owner 2026-09-06 選項①)──────────────
    // 三句話必須看得出是三件事。上一輪把「不存在」和「在,只是沒被載進來」講成
    // 同一句,owner 在驗收時被它騙過去 —— 所以這裡逐句點名「這是跟伺服器要過的
    // 結果」還是「條件把它擋掉了」。
    idMissing: (id: string) =>
      `找不到「${id}」。這是跟伺服器要過的結果，不是還沒載進來——這個編號現在不存在。`,
    idFilteredOut: (id: string, axes: string) =>
      `找到「${id}」了，但它不符合你目前的${axes}條件，所有條件是一起生效的，所以清單裡不會有它。`,
    idOnlyAgain: "只用編號再找一次",
  ...
  // ── 請示頁(M2 回覆卡 B2)──
  // ── 找一張票（T-93 第二輪：搜尋面板）──
  // owner 2026-09-06 c-2a36e68a5079 把第一輪的「邊打邊篩」換成「按下去才查」。
  // 這幾句的措辭是這個控制項的一半價值:idle / searching / miss / error 必須
  // 讀起來就不一樣,因為「還沒查」跟「查過但沒有」長得一樣正是它要修的病。
  search: {
    title: "找一張票",
    inputLabel: "任務或請示卡編號",
    placeholder: "貼上編號，例如 T-93 或 rc-dce83078a722",
    go: "搜尋",
    close: "關閉",
    hint: "貼上任務或請示卡編號，按「搜尋」。這裡不會邊打邊篩，按下去才去查。",
    searching: "查詢中…",
    // 🔴 這句要說出「查過了」,不能只說「沒有」——第一輪的空狀態就是因為說得像
    // 「沒有」而讓一個其實沒載進來的東西看起來像不存在。
    miss: (id: string) =>
      `找不到「${id}」。這是跟伺服器要過的結果，不是「還沒載進來」——這個編號現在不存在。`,
    error: "查不成：沒有問到伺服器。這不等於找不到，請再試一次。",
    errorUnknown: "未知的錯誤",
    kindCard: "請示卡",
    kindTask: "任務",
    cardStatus: {
    ...
    // 這個控制項做不到的事,寫在控制項上,不是寫在沒人會開的說明裡。
    foot: "編號要完整；打一半不會命中。目前只認編號，還不能搜標題或內容。",
  },
  // ── 篩選面板（T-93 第二輪）──
