...
export const zh = {
  ...
  settings: {
    ...
    stepNoteCapSub:
      "一個任務步驟的備註(step note)可以寫多少字——agent 換手時「做到哪、下一步接什麼」寫的就是這一格。範圍 1000~100000,可調高也可調低:這個上限只在寫入時檢查,所以調低不會弄丟任何東西,已經超過的舊備註照樣完整讀得回來,只是在寫短到上限以下之前改不動。它只管這一格:轉派任務的交接說明與聊天訊息本文仍然是寫死的 4000 字,不跟著這個設定走。",
    docUsage: "已用字數",
    chars: "字",
