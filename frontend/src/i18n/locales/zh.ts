...
export const zh = {
  ...
  lore: {
    ...
    // 🔴 本來三個，現在兩個。scopeKindRole（「角色傳承」）在 2026-09-07 隨著
    // 負責人收斂範圍一起拿掉（卡 rc-a43100fd0486 圈 [0]）。它沒有為了搬遷剩下
    // 的孤兒而保留：搬不動的那幾筆走的是 unknown 那一支，只顯示原始的鍵、不冠
    // 任何種類詞——因為對那種列唯一誠實的說法是「還沒有人決定它屬於誰」，把它
    // 叫成「角色傳承」等於指向一個這個產品已經沒有的範圍。
    //
    ...
    //
    // 🔴 四格的預設標籤一律用「所有」，不是「全部」。先前這一列兩個詞混著用
    // （全部撰寫人／所有任務／全部狀態），負責人那一句就是要把它統一；混用會
    // 讓讀的人以為兩個詞代表兩種不同的「沒有限制」。
    //
    // 🔴 成員那一格是「新加回來的一條路」，不是把撰寫人抄一份。撰寫人送的是
    ...
    //
    // ⚠️ 沒有統計數字，這是刻意的。任務頁的狀態會寫「狀態 · N」，這裡不寫
    // ——負責人明講「we dont need count」，而且傳承是往下捲載入的，這裡算得出
    // 來的只有「目前已載入幾筆」，會隨著捲動一直變。
    clearFilters: "清除篩選",
    filterAuthorNoun: "撰寫人",
    filterAuthorAll: "所有撰寫人",
    // 成員傳承這一格的選項＝現役成員名冊，跟撰寫人那一格同一份來源；名字用上
    // 面的 scopeKind* 那一組，所以篩選裡的字跟條目徽章上的字是同一份。
    filterMemberNoun: "成員傳承",
  ...
  tasks: {
    ...
    // ── 可結案:原本看不見的那個視窗(T-192) ─────────────────────────────────
    // 每個步驟都回報完成後,任務會停在 ready_for_done,而且會一直停在那裡——
    // 伺服器沒有計時器、也不會催任何人。卡片以前只把它當成另一個狀態字印出來,
    // 於是「等人按結案」跟「正在做」長得一模一樣。這一行就是在說:活做完了,
    // 但結案這個動作還沒發生。
    // 🔴 這一行被獨立審查退過兩次,兩次都是同一類錯:它描述了一個不存在的操作,
    // 或把能力指給了一個必定被拒絕的人。owner 2026-09-14 看過定稿後裁定:那些
    // 句子在解釋權限怎麼切,是寫給開發者的理由,不該出現在卡片上。所以這一行
    // 現在只講兩件事——步驟都完成了、在等負責人收尾——不再提一般結案歸誰、
    // 這個畫面上有沒有按鈕、也不再指路到「強制結案」。
    //
    // 還活著的那一個約束:等的是誰 = 這張票的「負責人」,而且要用卡片自己
    // 那個詞。一般結案(mark_task_done)的 callerMayMarkTaskDone 逐字是
    // `t.ExecutorID != "" && currentActor(r) == t.ExecutorID`——沒有 admin
    // 豁免,連 owner 都不行,所以「在等負責人」是真的,而「你可以收尾」會是
    // 假的。而這個 UI 裡指稱那個人的詞,從以前就是「負責人」(assigneeLabel /
    // filterExecutorNoun,見本檔 212、224 行),卡片的欄位列會直接印
    // 「負責人:某某」。舊版曾新發明「執行者」這個只在這一行出現過的名詞,於是
    // 同一張卡上兩個詞指同一個人,讀者無從得知——現在一律沿用「負責人」。
    // (那個命名本身不在這包的射程內,所以是這一行去對齊它,不是反過來。)
    //
    // 拿掉按鈕那句話不等於畫面上長出了按鈕:射程外的「結案」按鈕仍然不在,
    // frontend 產品碼裡也仍然沒有任何一處呼叫 api.markTaskDone。那件事現在
    // 只由 TaskCard.force-done.test.tsx 的 DOM 斷言守著,不再由文案複述。
    readyForDoneHint:
      "所有步驟都完成了,正在等負責人收尾。",
    // ── 強制結案(T-192) ────────────────────────────────────────────────────
    // 負責人不會再回來的票,只剩這條路出得去。只有 owner 與特助能按——真正
    ...
    forceDoneReasonPlaceholder: "為什麼在步驟還沒完成的情況下結案?",
    forceDoneConfirm: "確認強制結案",
    // 強制結案之後怎麼讀回來。被強制結案的票一定說得出來;理由可能沒有,
    // 沒有就顯示成沒有。
    forcedDoneLabel: "強制結案",
    forcedDoneNoReason: "未填理由",
    // 結案被拒絕時顯示:卡片要說出這張票「現在」在哪個狀態,因為只講「操作
    // 失敗」不足以讓 owner 判斷該重試、該展開卡片、還是什麼都不用做。
    //
    // 🔴 但「說出狀態」不等於「回答拒絕的原因」,而這一格被退回過一次。
    // HandleForceTaskDone 的拒絕只有三種:422(解碼／未知欄位)、404、409
    // (這張票已經是終態)。409 的充分必要條件就是「這張票已經結束了」,所以
    // 在那條路上,底下這句的前半「沒有被結案」跟後半「狀態是:已完成」永遠
    // 自相矛盾——而 409 是這個畫面上唯一真的會發生的拒絕。三種各自回答自己
    // 那個原因:
    closeStateError: "這張票沒有被結案。它現在的狀態是:",
    // 409:它已經結束了,所以「沒有被結案」是錯的說法——要講的是「不必再結
    // 一次」。狀態夾在中間(已完成/已終止/已標記重複),因為三種終態對讀者
    // 的下一步不一樣。
    closeAlreadyClosedLead: "這張票已經結束了(",
  ...
  settings: {
    ...
    taskCloseWinddown: "任務結案收尾秒數",
    taskCloseWinddownSub:
      "任務結案後，外包還能留著繼續收尾多久——四種結案方式都一樣。這段期間他在畫面上顯示「停止中」，要真的被收走才會從名單上消失；他自己回報收尾完成可以提早結束，但站台仍要先確認那個工作階段真的不在了。收尾期間他仍佔用一格外包併發額度，所以連續結案時新的發包會安靜地等在佇列裡（10–3600）",
    ...
    // T-33 的第三格。同一條規矩:一句,句號結束,不解釋位置、不寫留空、不寫上限
    // 數字。「傳承條目」而不是「傳承」:那一格的收件人是寫下那一條的人,不是
    // 一份文件。
    suggestedRepliesLoreMessage: "傳承訊息建議回覆",
    suggestedRepliesLoreMessageSub: "傳訊息給傳承條目的撰寫人時可一鍵帶入的句子。",
