...
export const zh = {
  ...
  lore: {
    ...
    qualityFlagged: (n: number) => `${n} 條沒有證偽條件、也沒有實例`,
    qualityWhy: "這一類條目就算被撈到也幫不上忙 —— 沒有辦法判斷它成不成立，也沒有一次真的發生過的事撐著它。",
    qualityShowOnly: "只看這些",
    qualityShowAll: "看全部",
    ...
    // ── 條目詳情 ──
    fieldSymptoms: "症狀 · 我當時看到什麼",
    fieldShort: "短版 · 只有這一段會進 agent 的記憶",
    fieldFalsify: "證偽條件 · 什麼情況代表這條不成立",
    fieldInstance: "實例 · 真的發生過的一次",
    fieldResidual: "殘餘風險 · 這條擋不住什麼",
    fieldEmpty: "（空白 —— 寫的人沒有填）",
    fieldsNote:
      "每一格都把欄位名印出來，空的也印。「空著」跟「沒有這一節」必須長得不一樣 —— 一條證偽條件與實例都空的記錄，一眼就看得出它是口號。",
