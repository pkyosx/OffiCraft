...
export const zh = {
  ...
  lore: {
    ...
    // 篩選列：撰寫人 → 屬於 → 狀態 → 清除篩選，抄任務頁的順序，少掉它那個
    // 任務編號搜尋框（負責人 2026-09-07，卡 rc-a43100fd0486：「你完全可以抄
    // task」）。
    //
    // 🔴 本來四格，現在三格。範圍／角色／手冊三個下拉都拿掉，由「屬於」一個
    // 取代：範圍的選項就是這次要收斂掉的那三種範圍，而它後面那兩份鍵清單是同
    // 一個問題問兩次。「屬於」把每一位成員與每一本任務手冊放在同一個複選裡；
    // 選項的字用上面 scopeKind* 那一組，所以篩選裡的名字跟條目列上的名字是同
    // 一份來源。
    //
