...
export const zh = {
  ...
  settings: {
    ...
    // 是兩件不同的對話,同一句話放錯格子就不合用(owner 裁定)。清單留空是合法
    // 的,代表那一邊的回覆框不顯示任何建議按鈕。
    suggestedRepliesReplyCard: "請示卡建議回覆",
    suggestedRepliesReplyCardSub:
      "請示卡回覆框下面那排一鍵回覆的句子。點一下只會把句子填進框裡,送出還是要自己按。留空就完全不顯示,回覆框跟沒有這個功能時一模一樣。最多 20 句,每句最多 120 字;超過會被拒絕存檔,不會被截短。",
    suggestedRepliesTaskMessage: "任務訊息建議回覆",
    suggestedRepliesTaskMessageSub:
      "任務卡「傳訊息給○○…」那個框下面的一鍵回覆句子。跟上面那格各自獨立——改這一份不會動到請示卡那一份。同樣最多 20 句、每句最多 120 字,留空就不顯示。",
