...
export const zh = {
  ...
  lore: {
    ...
    revisionFailed: "讀不到這一版，這是伺服器回的：",
      // ── 定位 (#lore/entry/<id>) 的三種結局,三句話,不可以合併 ──────────
      // (負責人 rc-428906235337,2026-09-05,原本是給任務頁的裁定,這裡照搬)
      //
      // 🔴 「找不到」跟「載不動」是相反的兩件事。404 是一個答案(這一條不存在),
      // 我們可以請他清除定位;500／離線是「沒有答案」,這時候給他一個出口按鈕,
      // 等於替一個我們沒問到的問題給了答案。
      anchorLoadFailed: "讀取那一條條目失敗(不是「查無此條目」——這一次我們沒問到答案)",
      // 錨點刻意不自己把 hash 拿掉:靜默 self-heal 會讓 404 一閃而過,只留下一份
      // 正常清單,而讀的人會以為自己看到了全部。
      anchorNotFound: (id: string) => `查不到這個條目:${id}。`,
      anchorClear: "清除定位",
    revisionsNote:
  ...
  mp: {
    ...
    loreActivity: {
      title: "傳承活動",
      loading: "載入中…",
      // 🔴 跟「這一任沒有取用紀錄」是相反的兩句話,永遠不可以合併:載入失敗
      // 時,我們對這個成員讀了什麼一無所知;而空清單是一句關於他的斷言。
      error: "讀取傳承活動失敗",
      retry: "重試",
      // 這一任「還沒開始／已結束」——沒有 session 錨,所以沒有這一任可言。
      // 對一個已下線的 agent 說「沒有取用紀錄」,是在怪他睡著的時候沒讀書。
      noSession: "這個成員目前沒有在跑,沒有「這一任」的取用紀錄可看。",
      // 四種空之中,只有這一句是在講這個 agent 做了什麼。
      empty: "這一任還沒有取用任何傳承條目。",
      // 開關關著時,歷史照樣看得到,只是沒有地方可以連過去(#lore 會被導回
      // 辦公室頁),所以標題不做成連結。
      loreOff: "傳承功能目前是關閉的,所以標題不能點開;下面這些是功能開著時留下的紀錄。",
      // 🔴 負責人指定的格式骨架:「上線後 N 分鐘 - Load Lore: <heading>」。
      // 單位是我們挑的(不到一分鐘用秒,免得 45 秒被寫成「0 分鐘」),骨架不動。
      afterBootMins: (n: number) => `上線後 ${n} 分鐘`,
      afterBootSecs: (n: number) => `上線後 ${n} 秒`,
      loadLore: "Load Lore:",
      retiredTag: "已退役",
      // ⚠️ 不可以寫「這一條被刪掉了」——2026-09-06 實測:全樹沒有任何
      // DELETE FROM lore_entry,退役也不是刪除。走到這裡代表異常,寫成「刪掉」
      // 會讓讀的人去找一個不存在的機制。
      headingGone: (id: string) => `查不到這個條目 id:${id}`,
    },
    resumeSummary: {
