...
export const zh = {
  ...
  tasks: {
    ...
    // 但結案這個動作還沒發生。
    // 🔴 兩種情況都要成立,所以它點了兩個角色的名。一般結案(mark_task_done)
    // 只有這張票「自己的執行者」按得動,owner 按下去是 403;而這張票存在的主
    // 要情境,正是執行者已經不會再回來(外包被釋出、成員離場),那時只講前半
    // 句就是一條沒有出口的死路。後半句講的就是出口與它在誰手上。舊版寫的是
    // 「正在等負責人按結案」——在最主要的那個情境裡它是假的,因為負責人根本
    // 按不到一般結案。
    readyForDoneHint:
      "所有步驟都已回報完成,這張票在等它的執行者按下結案;執行者已離場或卡住時,由負責人用「強制結案」收掉。",
    // ── 強制結案(T-192) ────────────────────────────────────────────────────
