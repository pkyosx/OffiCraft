...
export const zh = {
  ...
  tasks: {
    ...
    // 或把能力指給了一個必定被拒絕的人。定稿的三個約束,每一個都是量過的:
    //
    // (1) 等的是誰 = 這張票的「負責人」,而且要用卡片自己那個詞。一般結案
    //     (mark_task_done)的 callerMayMarkTaskDone 逐字是
    //     `t.ExecutorID != "" && currentActor(r) == t.ExecutorID`——沒有 admin
    //     豁免,連 owner 都不行。而這個 UI 裡指稱那個人的詞,從以前就是
    //     「負責人」(assigneeLabel / filterExecutorNoun,見本檔 212、224 行),
    //     卡片的欄位列會直接印「負責人:某某」。舊版新發明了「執行者」這個
    //     只在這一行出現過的名詞,於是同一張卡上兩個詞指同一個人,讀者無從
    //     得知——現在一律沿用「負責人」。(那個命名本身不在這包的射程內,
    //     所以是這一行去對齊它,不是反過來。)
    //
    // (2) 那個動作不在這個畫面上。這包把射程外的「結案」按鈕拿掉了,而且
    //     grep 過:frontend 產品碼裡沒有任何一處呼叫 api.markTaskDone(留在
    //     port 上的那支只被一支反向守衛 spy)。所以「在等它的負責人按下結案」
    //     會讓 owner 去找一顆不存在的按鈕——實際發生的事是負責人自己從它的
    //     工作階段呼叫結案。文案要描述後者。
    //
    // (3) 出口是「你」。強制結案的 route floor 是 Gated(principalAdminAgent),
    //     負責人本人在那道門上是 403(handler 註解逐字:"the task's own
    //     executor is a 403 HERE")。舊版寫「由負責人用『強制結案』收掉」,
    //     在中文讀者眼中就是「等這個人結案;這個人不在時,由這個人強制結案」。
    //     改用第二人稱,避開角色名詞,同時也是真的:駕駛艙只有一個 principal
    //     (AuthGate 的 `USE_MOCK || hasToken()` 與 viewerMayForceTaskDone 是
    //     同一個述詞),看得到這張卡的人就是那道門admits 的人。
    readyForDoneHint:
