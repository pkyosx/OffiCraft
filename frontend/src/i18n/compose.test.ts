...
const EXPECTED: [Lang, string, (string | number | string[] | number[])[], string][] = [
    ...
    ["zh", "loreScopeManualHint", ["PR 審查"], "之後任何人開「PR 審查」這本任務手冊時會讀到（正職、外包都一樣）。"],
    ["zh", "loreScopeAgentHint", ["Mira"], "只進 Mira 自己的開機檔，其他人讀不到。"],
    ...
    ["zh", "deleteRoleConfirm", ["研究員"], "確定刪除角色「研究員」？該角色的成員及其對話將一併移除，無法復原。"],
    ["zh", "docHistoryRestoreConfirm", ["7/29 14:03"], "確定還原「7/29 14:03」這個版本？目前的內容會被覆蓋，但會存成新的版本紀錄。"],
    ["zh", "docHistoryBlockedReason", [["判準", "SOP"], 10000], "「判準、SOP」超過 10000 字上限，且不比目前的內容短——伺服器會拒絕這次還原。"],
    ["zh", "deleteManualConfirm", ["review-pr"], "確定刪除任務類型「review-pr」？其手冊（定義、SOP）將一併移除，無法復原。"],
    ...
    ["en", "loreScopeManualHint", ["PR review"], "Anyone who opens the “PR review” task manual reads it from now on (staff and outsource alike)."],
    ["en", "loreScopeAgentHint", ["Mira"], "Only in Mira's own boot document; nobody else can read it."],
    ...
    ["en", "deleteRoleConfirm", ["研究員"], "Delete role \"研究員\"? Its members and their conversations will be removed permanently."],
    ...
    ["en", "docHistoryBlockedReason", [["Insight", "SOP"], 10000], "\"Insight, SOP\" is over the 10000-character limit and no shorter than what is stored now — the server would refuse this restore."],
    ["en", "deleteManualConfirm", ["review-pr"], "Delete the task type “review-pr”? Its manual (definition, SOP) is removed with it and cannot be restored."],
