...
## 請示卡頁的 ID 篩選（T-93，owner 2026-09-05 `rc-2085e5ec60be` / `rc-428906235337`）

`IdFilterInput` 是一個**普通篩選**，不是定位機制：不分大小寫的子字串比對，沒有「找不到」提示，除了下面那一次已處理載入之外**自己不打任何 API**。`#replies/card/<id>` 只是把 id **代入**這個篩選（外加捲動 focus），沒有它篩選一樣成立。

四格不變量，改這頁前先讀，每一格都有一支測試釘著（`RepliesPage.id-filter.test.tsx`）：

1. **篩選中要連帶把「近期已處理」載進來**，否則收合中的卡會被篩成「沒有符合」——一個假的空。
2. **那次載入一次 visit 只做一次**（`handledAutoloadTried`）：`loadHandled` 是兩支沒有 in-flight 去重的請求，載入旗標又要等它們回來才翻，沒有這道閂就變成**每敲一個字元發兩支請求**。而且**待回覆還在載入時不得觸發**——那時的空陣列意思是「還不知道」，不是「沒有命中」。
3. **篩選中那一段一律不隱藏，即使數字是 0。** zero-hide 會把整段連同展開的把手一起刪掉，於是「沒有符合」與「這一段不存在」在畫面上長得一樣。
4. **標題的「· N」只有已處理已載入時才改用篩過的長度**；沒載入時用 server 的整段計數。

空狀態有兩句，**不可合併**：沒有篩選時是「✓ 目前沒有待處理的請示」，篩不到時是「沒有符合篩選條件的請示」。後者若說成前者，會在六張卡等著時告訴 owner 他已經回完了。

⚠️ **射程**：待回覆那一段沒有時間窗，多久以前開的都篩得到；24 小時的窗只切已回覆與已過期。篩不到的只有「已回覆或已過期且超過 24 小時」的卡——這是 owner 具名接受的代價（2026-09-05 `c-0e183a7fbb10`），**不要為它偷加一支 by-id 讀取**。

⚠️ `matchesId` 的 effect 依賴是手工維護的，這個 repo 沒有 ESLint ⇒ 加第二個篩選軸時它會安靜地 stale。
